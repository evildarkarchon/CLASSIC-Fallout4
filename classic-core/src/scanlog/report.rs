...
impl ReportFragment {
    ...
    fn py_new(lines: Option<Vec<String>>, check_content: bool, use_pool: bool) -> Self {
        match lines {
            Some(lines) if use_pool => Self::from_lines_pooled(lines, &STRING_POOL),
            Some(lines) => {
                let has_content = if check_content {
                    !lines.is_empty()
                } else {
                    true
                };
...
impl ParallelReportProcessor {
    ...
    fn transform_fragments(
        ...
        fragments
            .par_iter()
            .map(|fragment| match operation.as_str() {
                "uppercase" => {
                    let upper_lines: Vec<String> =
                        fragment.content.iter().map(|s| s.to_uppercase()).collect();
                    ReportFragment::from_lines(upper_lines)
                }
                "trim" => {
                    let trimmed_lines: Vec<String> = fragment
                        .content
                        .iter()
