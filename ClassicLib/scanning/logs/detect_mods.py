...
def _get_report_fragment() -> type[ReportFragment]:
    """Lazy import ReportFragment to avoid circular dependency."""
    from ClassicLib.integration.rust.report_rust import ReportFragment
...
def detect_mods_single(yaml_dict: dict[str, str], crashlog_plugins: dict[str, str]) -> ReportFragment:
    ...
    if not mod_names:
        return _get_report_fragment().empty()
    ...
    return _get_report_fragment().from_lines(lines)
...
def detect_mods_important(
    ...
    RF = _get_report_fragment()
    return RF.from_lines(lines, check_content=False) if lines else RF.empty()
