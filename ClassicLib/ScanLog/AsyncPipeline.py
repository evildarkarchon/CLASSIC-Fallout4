...
class AsyncCrashLogPipeline:
    ...
    async def process_crash_logs_async(
        ...
        cache_dict = {
            ...
            for name, lines in log_cache_data.items()
        }
        ...
        reports_to_write = [
            ...
            for crashlog_file, autoscan_report, trigger_scan_failed, _ in all_results
        ]
        ...
        logger.info(f"Async pipeline completed: {len(crashlog_list)} logs in {self.performance_stats['total_time']:.3f}s "
...
async def run_async_crash_log_scan(  # noqa: PLR0913
    ...
    pipeline = AsyncCrashLogPipeline(
        yamldata, fcx_mode, show_formid_values, formid_db_exists
    )
...
async def benchmark_async_pipeline(  # noqa: PLR0913
    ...
    comparison = AsyncPerformanceMonitor.compare_performance(
        async_stats, sync_baseline or 0, len(crashlog_list)
    )
