...
async def async_crashlogs_scan() -> None:
    ...
    # Load settings
    remove_list: tuple[str] | tuple[str] = yaml_settings(tuple[str], YAML.Main, "exclude_log_records") or ("",)
    ...
    async with AsyncScanOrchestrator(
        yamldata, crashlogs, fcx_mode, show_formid_values, formid_db_exists
    ) as orchestrator:
