...
class ReportGenerator:
    ...
    def generate_header(self, crashlog_filename: str, autoscan_report: list[str]) -> None:
        ...
        append_or_extend(
            (
                f"# {crashlog_filename}\n",
                f"**AUTOSCAN REPORT GENERATED BY {self.yamldata.classic_version}**\n\n",
        ...
        append_or_extend(
            (
                ...
                (
                    ...
                    else f"⚠️ {self.yamldata.warn_outdated}\n\n"
                ),
            ),
            autoscan_report,
        )
    ...
    def generate_footer(self, autoscan_report: list[str]) -> None:
        ...
        if GlobalRegistry.get_game().replace(" ", "") == "Fallout4":
            insert_text = ("\n", self.yamldata.autoscan_text)
            append_or_extend(insert_text, autoscan_report)
        append_or_extend(f"\n*{self.yamldata.classic_version} | {self.yamldata.classic_version_date} | END OF AUTOSCAN*\n", autoscan_report)
        ...
        if trigger_plugin_limit and not trigger_limit_check_disabled and trigger_plugins_loaded:
            ...
            append_or_extend(
                (
                    ...
                    "** Recommendation: Consider updating Buffout 4 NG to the latest version. ** \b\n-----\n",
                ),
                autoscan_report,
            )
