...
full compatibility with the Parser API.
"""
...
def extract_segments(crash_data: list[str], segment_boundaries: list[tuple[str, str]], eof_marker: str) -> list[list[str]]:
    ...
    and extracts segments defined by start and end boundaries.
...
def extract_module_names(module_texts: set[str]) -> set[str]:
    ...
    name, it is ignored while extracting the name.

    Args:
        module_texts: A set of strings containing module names and optional metadata.
