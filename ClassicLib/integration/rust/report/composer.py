...
_PyReportComposer: type | None = None
...
def _get_py_report_composer() -> type:
    """Lazily import PyReportComposer to avoid circular imports."""
    global _PyReportComposer  # noqa: PLW0603 - Intentional lazy initialization pattern
    if _PyReportComposer is None:
        from ClassicLib.scanning.logs.reporting.section_composer import ReportComposer

        _PyReportComposer = ReportComposer
    return _PyReportComposer
...
if TYPE_CHECKING:
    ...
    from ClassicLib.integration.rust.report.fragment import RustAcceleratedReportFragment as _FragmentType
...
class RustAcceleratedReportComposer:
    ...
    def __init__(self, _parallel_threshold: int = 10) -> None:
        ...
        if self._use_rust:
            ...
            self._composer = _get_py_report_composer()()
    ...
    def add(self, fragment: Any) -> RustAcceleratedReportComposer:
        ...
        if self._use_rust:
            ...
            elif isinstance(fragment, _get_py_report_fragment()):
        ...
        # Try to convert
            ...
            py_frag = _get_py_report_fragment().from_lines(fragment.to_list())
