...
def _determine_relevant_versions(
    ...
    if is_vr_mode:
        ...
        wrong_versions = [ALL_ADDRESS_LIB_INFO["OG"], ALL_ADDRESS_LIB_INFO["NG"]]
    else:
...
def check_xse_plugins() -> str:
    ...
    correct_version_exists = any(
        plugins_path.joinpath(version["filename"]).exists() for version in correct_versions
    )
    wrong_version_exists = any(
        plugins_path.joinpath(version["filename"]).exists() for version in wrong_versions
    )
