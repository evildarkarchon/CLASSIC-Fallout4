...
class TestFullReportParity:
    ...
    def test_generate_error_section_semantic_parity(self, rust_report_generator_with_config):
        ...
        rust_error = rust_report_generator_with_config.generate_error_section("EXCEPTION_ACCESS_VIOLATION", "1.37.0", False)
        ...
        rust_text = "\n".join(rust_content)
        assert "EXCEPTION_ACCESS_VIOLATION" in rust_text or "Error" in rust_text, "Error section should reference the main error"
        assert "1.37.0" in rust_text, "Error section should reference crashgen version"

        # Should NOT have outdated warning when is_outdated=False
        assert "OUTDATED" not in rust_text.upper(), "Rust should NOT show outdated warning when is_outdated=False"
    ...
    def test_generate_error_section_outdated_warning(self, rust_report_generator_with_config):
        ...
        rust_error = rust_report_generator_with_config.generate_error_section("EXCEPTION_ACCESS_VIOLATION", "1.30.0", True)

        rust_content = normalize_content(rust_error.to_list())
        rust_text = "\n".join(rust_content)

        # Should have outdated warning when is_outdated=True
        assert "OUTDATED" in rust_text.upper() or "WARNING" in rust_text.upper(), "Rust should show outdated warning when is_outdated=True"
    ...
    def test_suspect_found_footer_exact_match(self, rust_report_generator):
        ...
        assert normalize_content(rust_none.to_list()) == normalize_content(py_none.to_list()), "Suspect found footer mismatch (found=False)"
    ...
    def test_report_section_structure_matches_sample(self, rust_report_generator):
        ...
        assert any("AUTOSCAN REPORT GENERATED BY" in line for line in header_lines), "Header should contain 'AUTOSCAN REPORT GENERATED BY'"
        assert any("FOR BEST VIEWING EXPERIENCE" in line for line in header_lines), "Header should contain viewing instructions"
        assert any("BEWARE OF FALSE POSITIVES" in line for line in header_lines), "Header should contain false positive warning"
        assert any("---" in line for line in header_lines), "Header should contain markdown separator"

        # Validate section headers use proper markdown format
        suspect_header = rust_report_generator.generate_suspect_section_header()
        assert any("###" in line for line in suspect_header.to_list()), "Suspect section should use markdown H3 format"

        settings_header = rust_report_generator.generate_settings_section_header()
        assert any("###" in line for line in settings_header.to_list()), "Settings section should use markdown H3 format"
        ...
        suspect_footer = rust_report_generator.generate_suspect_found_footer(True)
        assert any("---" in line for line in suspect_footer.to_list()), "Section footers should include markdown separators"
