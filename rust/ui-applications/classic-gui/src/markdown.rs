...
Generated by CLASSIC v8.2.0";
        ...
        assert!(
            headings.iter().any(|h| h.contains("Error Information")),
            "Missing 'Error Information'. Headings found: {:?}",
            headings
        );
        assert!(
            headings.iter().any(|h| h.contains("Crash Messages")),
            "Missing 'Crash Messages'. Headings found: {:?}",
            headings
        );
        assert!(
            headings.iter().any(|h| h.contains("Settings-related")),
            "Missing 'Settings-related'. Headings found: {:?}",
            headings
        );
        assert!(
            headings.iter().any(|h| h.contains("HAVE SOLUTIONS")),
            "Missing 'HAVE SOLUTIONS'. Headings found: {:?}",
            headings
        );
        assert!(
            headings.iter().any(|h| h.contains("Important Mods")),
            "Missing 'Important Mods'. Headings found: {:?}",
            headings
        );
        assert!(
            headings.iter().any(|h| h.contains("Plugin-related Errors")),
            "Missing 'Plugin-related Errors'. Headings found: {:?}",
            headings
        );
        assert!(
            headings.iter().any(|h| h.contains("Named Records")),
            "Missing 'Named Records'. Headings found: {:?}",
            headings
        );
        assert!(
            headings.iter().any(|h| h.contains("End of Report")),
            "Missing 'End of Report'. Headings found: {:?}",
            headings
        );
        ...
        for block in &star_pattern_blocks {
            assert_ne!(
                block.block_type, BLOCK_LIST_ITEM,
                "Star-wrapped text should NOT be a list item: {:?}",
                block.text
            );
            assert!(
                !block.text.ends_with(" *") && !block.text.ends_with("*"),
                "Star-wrapped text should NOT have trailing asterisk: {:?}",
                block.text
            );
        }
        ...
        for cb in &code_blocks {
            assert!(
                !cb.text.trim().starts_with("-----"),
                "Indented ----- should be a thematic break, not code: {:?}",
                cb.text
            );
        }
    }
    ...
    fn test_normalize_star_patterns() {
        ...
        assert!(
            ...
            "Should strip leading asterisk framing: {:?}",
            normalized
        );
        assert!(
            normalized.contains("**SUSPECTS DETECTED!**"),
            "Should preserve inner bold content: {:?}",
            normalized
        );
        ...
        assert!(
            list_normalized.contains("* Item one"),
            "Should NOT strip real list items: {:?}",
            list_normalized
        );
    }
    ...
    fn test_normalize_thematic_break_before_heading() {
        ...
        assert!(
            normalized.contains("-----\n\n### My Heading"),
            "Should add blank line between thematic break and heading: {:?}",
            normalized
        );
    }
    ...
    fn test_normalize_indented_separator() {
        ...
        assert_eq!(
            normalized.trim(),
            "-----",
            "Indented separator should be unindented: {:?}",
            normalized
        );
    }
    ...
    fn test_normalize_indented_separator_with_trailing_whitespace() {
        ...
        assert_eq!(
            normalized.trim(),
            "-----",
            "Indented separator with trailing whitespace should be unindented: {:?}",
            normalized
        );
    }
    ...
    fn test_normalize_preserves_regular_content() {
        ...
        assert!(
            normalized.contains("Some paragraph text"),
    ...
    fn test_normalize_three_dash_before_heading() {
        ...
        assert!(
            normalized.contains("---\n\n### My Heading"),
            "Should add blank line between --- and heading: {:?}",
            normalized
        );
    }
    ...
    fn test_normalize_star_pattern_not_matching_list_with_sub_items() {
        ...
        assert!(
            ...
            "Should NOT strip list items with sub-items: {:?}",
            normalized
        );
    }
}
