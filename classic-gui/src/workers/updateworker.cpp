...
void UpdateWorker::checkForUpdates(const QString& currentVersion)
{
    try {
        ...
        if (!result.error_message.empty()) {
            emit updateCheckCompleted(false, QString(), classic::toQString(result.error_message));
        } else {
            emit updateCheckCompleted(result.has_update, classic::toQString(result.latest_version), QString());
        }
