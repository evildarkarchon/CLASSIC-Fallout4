...
BackupController::BackupController(const QString& gameRoot,
                                   SignalHub* signalHub,
                                   QObject* parent)
    : QObject(parent)
    , m_gameRoot(gameRoot)
...
bool BackupController::backupExists(const QString& backupType) const {
    ...
    try {
        auto mgr = classic::files::backup_manager_new(
            classic::toRustString(m_gameRoot));
        return classic::files::backup_manager_exists(
            *mgr, classic::toRustString(backupType));
...
void BackupController::backup(const QString& backupType) {
    ...
    try {
        auto mgr = classic::files::backup_manager_new(
            classic::toRustString(m_gameRoot));
        auto result = classic::files::backup_manager_create(
            *mgr, classic::toRustString(backupType));
        QString msg = backupType.toUpper()
            + QStringLiteral(" backup created: ")
            + classic::toQString(result);
...
void BackupController::restore(const QString& backupType) {
    ...
    try {
        auto mgr = classic::files::backup_manager_new(
            classic::toRustString(m_gameRoot));
        auto count = classic::files::backup_manager_restore(
            *mgr, classic::toRustString(backupType));
        QString msg = backupType.toUpper()
...
void BackupController::remove(const QString& backupType) {
    ...
    try {
        auto mgr = classic::files::backup_manager_new(
            classic::toRustString(m_gameRoot));
        classic::files::backup_manager_remove(
            *mgr, classic::toRustString(backupType));
        QString msg = backupType.toUpper()
            + QStringLiteral(" backup removed");
        emit operationCompleted(msg);
    } catch (const rust::Error& e) {
        emit operationError(
            backupType.toUpper()
