...
#[test]
fn empty_decorative_framing_does_not_panic() {
    assert_eq!(normalize_markdown("* *"), "* *");
}
#[test]
fn writes_markdown_conformance_receipt() {
    crate::tests::interface_receipt("markdown-rendering", |fixture| {
        let input = fixture["markdown"].as_str().expect("markdown input");
        let normalized = normalize_markdown(input);
        let html = markdown_to_html(input);
        let complete = html.starts_with("<html><head><style>") && html.ends_with("</body></html>");
        let body = html
            .split_once("</style></head><body>")
            .expect("body opening")
            .1
            .strip_suffix("</body></html>")
            .expect("body closing");
        serde_json::json!({"normalized": normalized, "htmlBody": body, "documentComplete": complete})
    });
}
