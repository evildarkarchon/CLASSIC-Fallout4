...
**Crashgen Settings Analysis**:
The report-relevant evaluation of one Crash Log's Crashgen settings. It produces YAML Data-backed Crashgen Expectation Outcomes and separate universal Disabled Setting Notices.
_Avoid_: settings validation report, named checks, hardcoded settings scan
...
The arrangement of scan findings and guidance into the canonical Autoscan Report structure for human review. It determines section order, finding placement, presentation, and code-authored report text while preserving authored guidance supplied by Autoscan Report Contributions.
...
An item in an Autoscan Report Contribution that identifies crash evidence matching one known crash message, stack pattern, or DLL crash clue. Each matched rule or DLL involvement notice is one Crash Suspect Finding; it signals a suspected cause, not a guaranteed root cause.
