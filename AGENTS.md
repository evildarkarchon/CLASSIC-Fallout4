...
`.agents/skills/` is the source of truth for every project skill. Other tool directories may carry mirrors — `.cursor/skills/` currently mirrors the OpenSpec skills, and `.opencode/skills/` mirrors `classic-project-guide`. When you change a skill under `.agents/skills/`, refresh its mirrors in the same change or they silently rot.
