...
Rules:
- Graphify must use the OpenAI backend only. The Claude/Anthropic backend is forbidden, and agents must not install or use the `anthropic` package for graphify.
