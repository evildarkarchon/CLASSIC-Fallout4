...
- Belt-and-suspenders for the above: `cmake/ClassicLinkerCheck.cmake` is included by both `classic-gui/CMakeLists.txt` and `classic-cli/CMakeLists.txt` and fails the configure with `FATAL_ERROR` if the first `link.exe` on PATH is not the MSVC linker (behavioral probe, not a path heuristic). The error names the offending binary and points back to the remediation above; if you see it, fix the shell environment rather than disabling the guard.
