...
- Add Results tab which lists all the reports generated by CLASSIC with a viewer to read the report right in CLASSIC.
- Add window size persistence on a per-tab basis.
- Many markdown compliance fixes for the reports.
- Some sections of the report will now be hidden in the report if they have no content.
