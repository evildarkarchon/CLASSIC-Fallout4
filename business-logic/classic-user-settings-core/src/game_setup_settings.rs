...
const DEFAULT_MANAGED_GAME: GameId = GameId::Fallout4;
...
impl GameSetupSettings {
    ...
    pub(crate) fn from_nested_document(
        ...
        let managed_game = managed_game_preference(&group["Managed Game"], &mut diagnostics);
        let game_root = optional_absolute_path_preference(
            &group["Game Folder Path"],
        ...
        let documents_paths = aliased_optional_absolute_path_preference(
            &group["Documents Folder Path"],
            &group["INI Folder Path"],
            OptionalPathField::new(
    ...
    pub(crate) fn from_legacy_flat_document(
        ...
        (
            Self {
                managed_game: Preference::new(DEFAULT_MANAGED_GAME, PreferenceOrigin::Default),
                game_version_selection: Preference::new(
                    scan.game_version_selection(),
                    scan.game_version_selection_origin(),
                ),
                game_root,
                game_executable: Preference::new(None, PreferenceOrigin::Default),
                documents_root: documents_paths.resolved,
                ini_folder: documents_paths.alias,
                mods_root,
                custom_scan_input: Preference::new(
                ...
                papyrus_log: Preference::new(None, PreferenceOrigin::Default),
            },
            diagnostics,
        )
    }
    ...
    fn with_origin(origin: PreferenceOrigin) -> Self {
        Self {
            managed_game: Preference::new(DEFAULT_MANAGED_GAME, origin),
            game_version_selection: Preference::new(GameVersionSelection::Auto, origin),
            game_root: Preference::new(None, origin),
            game_executable: Preference::new(None, origin),
            documents_root: Preference::new(None, origin),
            ini_folder: Preference::new(None, origin),
            mods_root: Preference::new(None, origin),
            custom_scan_input: Preference::new(None, origin),
            papyrus_log: Preference::new(None, origin),
        }
    }
}
...
fn managed_game_preference(node: &Yaml, diagnostics: &mut Vec<Diagnostic>) -> Preference<GameId> {
    ...
    if matches!(node, Yaml::BadValue) {
        return Preference::new(DEFAULT_MANAGED_GAME, PreferenceOrigin::Default);
    }
    ...
    Preference::new(DEFAULT_MANAGED_GAME, PreferenceOrigin::DegradedFallback)
}
