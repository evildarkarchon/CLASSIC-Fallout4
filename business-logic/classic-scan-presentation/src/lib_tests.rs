...
fn rendered_corpus() -> Vec<DisplayLine> {
    ...
    // The Local Ignore recovery prompt, in both shapes it takes. Its decision descriptions
    // are segment lists rather than lines, so they are wrapped as lines here: the invariant
    // tests below read segments, and prose reaching a user through a description must be held
    // to the same rule as prose reaching one through a line.
    //
    // The wording itself is pinned in `recovery_tests.rs`, beside the module that renders it.
    // What this adds is coverage by the crate-wide invariants, which no per-module test file
    // can give it — a description that grew a placeholder or a re-derived plural would escape
    // every check in this crate if the prompt were absent from this corpus.
    let mut prompts = vec![render_local_ignore_recovery(None)];
    for reset_available in [true, false] {
        prompts.push(render_recovery_prompt(reset_available));
    }
