...
impl PyPluginAnalyzer {
    ...
    pub fn loadorder_scan_log(&self, segment_plugins: Vec<String>) -> PyResult<Vec<String>> {
        self.inner.loadorder_scan_log(segment_plugins).map_err(crate::to_pyerr)
    }

    /// Check plugin limit - returns (plugin_limit_triggered, limit_check_disabled)
    pub fn check_plugin_limit(&self, segment_plugins: Vec<String>, game_version: String, version_current: String) -> PyResult<(bool, bool)> {
        self.inner.check_plugin_limit(segment_plugins, &game_version, &version_current).map_err(crate::to_pyerr)
    }

    /// Match plugins
    pub fn plugin_match(&self, segment_callstack_lower: Vec<String>, crashlog_plugins_lower: HashSet<String>) -> PyResult<Vec<String>> {
        self.inner.plugin_match(segment_callstack_lower, crashlog_plugins_lower).map_err(crate::to_pyerr)
    }

    /// Filter ignored plugins
    pub fn filter_ignored_plugins(&self, plugins: HashMap<String, String>) -> PyResult<HashMap<String, String>> {
        self.inner.filter_ignored_plugins(plugins).map_err(crate::to_pyerr)
    }
}
