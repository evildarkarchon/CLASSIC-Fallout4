...
def observe_update_services(fixture: dict[str, Any], scenario_id: str) -> dict[str, Any]:
    ...
    config = json.dumps({"github_api_base_url": base + "/api", "notification_pages_url": base + "/pages",
                         "timeout_ms": fixture["timeoutMs"]})
    with tempfile.TemporaryDirectory(prefix="classic-update-") as directory:
        ...
        for _ in range(fixture["checks"]):
            try:
                status = classic_update.check_app_notification_configured("conformance", "updates",
                                                                          fixture["installedVersion"], config,
                                                                          str(cache))
                ...
                results.append({"status": {
                    ...
                    "display": None if display is None else {"title": display.title, "body": display.body,
                                                             "ctaUrl": display.cta_url},
        ...
        files = [{"path": path.relative_to(directory).as_posix(), "hex": path.read_bytes().hex()} for path in
                 sorted(cache.rglob("*")) if path.is_file()]
