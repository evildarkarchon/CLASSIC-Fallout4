"""Regression guard: regenerating the Node baseline twice must not churn.
...
The gates never caught this because their comparator ignores the timestamp on
both sides. These tests assert the stronger property the baselines actually
need: two runs against unchanged sources produce byte-identical files,
timestamp included.
"""
...
GENERATE_BASELINE = REPO_ROOT / "tools" / "node_api_parity" / "generate_baseline.py"
