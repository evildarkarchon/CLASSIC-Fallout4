...
- [ ] 10.18 Run the binding-local Python smoke tests: `uv run --python python-bindings/.venv/Scripts/python.exe python -m pytest python-bindings/tests -q`. *(ENV ISSUE, NOT TEST FAILURE: the binding-local virtualenv at `python-bindings/.venv/` does not have the maturin-built extension modules (`classic_shared`, `classic_version_registry`, etc.) installed — 9 collection errors reported as `ModuleNotFoundError`. Reproducible on pre-split HEAD (`git stash` the split then re-run pytest → identical `ModuleNotFoundError` failures). This is a pre-existing virtualenv setup gap unrelated to the test-module-layout change; to fix, the maintainer needs `maturin develop --manifest-path python-bindings/classic-<name>-py/Cargo.toml` for each binding before running pytest. Leaving this task unchecked so the maintainer sees it, but marking the spec-relevant invariant (Python parity gate) as passed via 10.17.)*
