...
Declare the `schema_version` contract that every shippable CLASSIC YAML file carries, and the compile-time-visible compatibility ranges each client build accepts for each file family. Together, these gate YAML loads so an older client never silently parses a newer file with a different shape.
