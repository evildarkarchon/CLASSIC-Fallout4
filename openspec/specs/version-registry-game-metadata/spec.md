...
The `GameVR_Info` section SHALL NOT exist in the game database YAML files (`CLASSIC Fallout4.yaml`). All data previously in `GameVR_Info` SHALL be accessible through the Version Registry's `FO4_VR` entry. The `Game_Info` section in the database YAML SHALL NOT contain version-specific static metadata (versions, hashes, XSE info, crashgen info). Non-version fields (backup lists, hints, warnings, defaults) remain in the YAML at their current top-level keys.
