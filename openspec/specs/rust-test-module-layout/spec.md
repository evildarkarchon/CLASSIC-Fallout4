# Rust Test Module Layout Specification

## Purpose

Define the canonical layout rule for in-source Rust unit tests across the CLASSIC workspace: every source file keeps its tests in a sibling `<name>_tests.rs` file rather than in an inline `#[cfg(test)] mod tests { ... }` block. This rule applies uniformly across `foundation/`, `business-logic/`, `cpp-bindings/classic-cpp-bridge/`, `node-bindings/classic-node/`, `python-bindings/`, `ui-applications/classic-tui/`, and any future Rust crate under those layer roots. It exists so unit tests are physically colocated with the code they exercise while staying out of the production-source file, so relocation preserves cargo-reported test identity and binding parity baselines, and so new modules and crates scaffold with the sibling layout from the start. Integration tests (`tests/`), benches (`benches/`), and dedicated `tests/common/` helper modules are out of scope and stay where they are.
