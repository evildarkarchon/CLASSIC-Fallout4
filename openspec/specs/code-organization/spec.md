...
### Requirement: Documentation Completeness for Return Values
All public functions with non-None return types SHALL include a `Returns:` section in their Google-style docstrings. This ensures complete documentation of function behavior and return values.

#### Scenario: Function with return value has Returns section
- **WHEN** a public function has a return type annotation other than `None`
- **THEN** the docstring MUST include a `Returns:` section
- **AND** the Returns section MUST describe what the function returns
- **AND** the description MUST match the actual return type

#### Scenario: Function returning None omits Returns section
- **WHEN** a public function has `-> None` or no return type
- **THEN** the docstring SHOULD NOT include a `Returns:` section
- **AND** side effects SHOULD be documented in the main description

#### Scenario: Async function with return value
- **WHEN** an async function (coroutine) has a non-None return type
- **THEN** the `Returns:` section MUST document the awaited result type
- **AND** the description MAY note async behavior if relevant

#### Scenario: Property with return value
- **WHEN** a `@property` decorated method returns a value
- **THEN** the docstring MUST include a `Returns:` section
- **AND** the description MUST document what the property represents

#### Scenario: Generator or iterator function
- **WHEN** a function yields values (generator)
- **THEN** the docstring MUST use `Yields:` section instead of `Returns:`
- **AND** each yielded value type MUST be documented
