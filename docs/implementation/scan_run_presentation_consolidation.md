...
- **`recovery.rs` carries its tests in `recovery_tests.rs`**, per rule 11, rather than in `lib_tests.rs` beside the crate-wide invariants. The corpus in `lib_tests.rs` still renders the prompt, which is deliberate and not a duplicate: the wording is pinned once beside the module, while the corpus is what subjects the decision descriptions to the placeholder and grammatical-number invariants that no per-module file can apply. `display.rs` and `render.rs` still have no siblings; moving those is a larger change than this ticket.
...
- **The prompt states the question, not the facts behind it.** `lines` originally re-rendered the malformed file's Local Ignore line, which put the same line on screen twice in all three interactive frontends — the drift this whole effort removes, merely relocated into core. It was dropped once the frontends landed and the duplication became visible: `render_run_result` already carries that line in the Installed YAML Data block, and every surface receiving a prompt receives the rendered run on the same envelope, so the identity is stated once rather than never. That reduced `render_recovery_prompt` to a single `bool` parameter.
