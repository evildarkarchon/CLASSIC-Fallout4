...
│  - classic-database-core: FormID lookups            │
...
impl CliConfig {
    ...
    pub async fn load_from_yaml(path: &Path) -> Result<Self> {
        // Uses classic-yaml-core directly
    }
...
pub struct ScanExecutor {
    config: CliConfig,
    yaml_data: YamlData,  // From classic-config-core
}
...
impl ScanExecutor {
    pub async fn scan(&self) -> Result<ScanResults> {
        ...
        // 2. Parse logs (classic-scanlog-core)
        let parser = LogParser::new();
        let parsed = parser.parse_logs(&logs).await?;

        // 3. Analyze FormIDs (classic-database-core)
...
- Python: Load via PyO3 bindings (classic-config-py)
...
# Linux/macOS
upx --best --lzma target/release/classic-cli
upx --best --lzma target/release/classic-tui
```
