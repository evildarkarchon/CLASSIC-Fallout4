...
| `uv venv ClassicLib-rs/python-bindings/.venv` | `uv venv python-bindings/.venv` | Keep the bindings-local virtual environment; only the path root changed. |
| `uv pip install --python ClassicLib-rs/python-bindings/.venv/Scripts/python.exe -r ClassicLib-rs/python-bindings/requirements-ci.txt` | `uv pip install --python python-bindings/.venv/Scripts/python.exe -r python-bindings/requirements-ci.txt` | Install Python binding CI dependencies into the binding-local virtualenv. |
| `uv run --python ClassicLib-rs/python-bindings/.venv/Scripts/python.exe python -m pytest ClassicLib-rs/python-bindings/tests -q` | `uv run --python python-bindings/.venv/Scripts/python.exe python -m pytest python-bindings/tests -q` | Binding smoke tests use the moved path. |
