...
All three bindings propagate a single unified `UpdateError` family (design decision D-05) with notification-specific variants `NotificationFetchFailed`, `NotificationDecode`, `NotificationInstalledVersionParse`, `NotificationCacheIo`. The shape each binding exposes follows the established per-language idiom:
...
| Python (PyO3) | Raises a `ClassicNotificationError` subclass keyed to the variant; subclass of `ClassicUpdateError` so existing catches still work |
