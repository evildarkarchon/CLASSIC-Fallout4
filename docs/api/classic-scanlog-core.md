...
- Cross-interface behavior is pinned by `tests/fixtures/crash_log_scan_run/manifest.json` and its immutable YAML/Crash Log corpus. Rust, the native CXX bridge, Node, and Python execute the same normalized Standard and Targeted cases; the binding compliance suite fails when a Rust contract enum gains a variant that any supported adapter has not acknowledged. CLI, GUI, and TUI presentation tests are registered as evidence that their messages and progress consume the Rust-owned discovery, concurrency, cancellation, failure, and terminal-order facts.
