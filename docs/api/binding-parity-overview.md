...
**Installed YAML Data load contract (`classic-config-core`):** Rust, C++, Node, and Python load one installation root, typed game, and separate selected-game-version mode into one of three typed states: Ready, Local Ignore Recovery Required, or fatal error. Missing Local Ignore is strictly validated from retained selected-Main defaults, atomically published without clobbering a concurrent winner, and reread from the authoritative canonical file. Malformed existing bytes are retained with the selected Main/game snapshot, default availability, identity, mode, and diagnostics in an opaque recovery plan. Valid defaults expose an identity on every surface; unavailable defaults do not block Proceed Without Ignore. Every surface exposes a consuming Proceed Without Ignore operation that returns the retained snapshot with an empty operation-scoped ignore list and no filesystem mutation; the next load therefore requires recovery again. Ready snapshots expose parsed YAML Data, independently selected Main/game provenance and schema, exact identities, `Existing` or `Generated` state, and structured fallback/`LocalIgnoreGenerated` diagnostics without raw bytes or YAML documents. Proceeded snapshots use `ProceedWithoutIgnore` and retain the malformed identity for attribution. Entry points are CXX `installed_yaml_data_load(...)` plus its status/snapshot/recovery-plan functions, Node `loadInstalledYamlData(...)`, and Python `classic_config.load_installed_yaml_data(...)`.
