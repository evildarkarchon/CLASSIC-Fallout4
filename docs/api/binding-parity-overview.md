...
**Installed YAML Data load contract (`classic-config-core`):** Rust, C++, Node, and Python load one installation root, typed game, and separate selected-game-version mode into Ready, Local Ignore Recovery Required, or fatal error. Missing Local Ignore is strictly validated from retained selected-Main defaults, atomically published without clobbering a concurrent winner, and reread from the authoritative canonical file. Malformed bytes are retained with the selected Main/game snapshot, defaults, identity, mode, and diagnostics in an opaque single-use plan. Every surface exposes both decisions: Proceed Without Ignore returns the retained snapshot with an empty operation-scoped list and no filesystem mutation; Reset To Default conflict-checks exact bytes, durably publishes and verifies a byte-exact backup, and atomically publishes retained defaults without `.prev` update-channel state. Reset success returns canonical/backup paths, malformed/backup/replacement identities, the reset-ready snapshot, and structured reset diagnostics; changed or removed canonical bytes return a typed conflict, while pre-publication failures retain stable stage metadata. Post-rename durability uncertainty is a separate typed outcome carrying canonical and verified-backup paths plus all three identities. The synchronous Rust reset is the non-interruptible critical section; Node runs it on its worker pool and Python releases the GIL. Snapshots expose parsed YAML Data, simplify-log removal entries retained from selected Main, independently selected Main/game provenance and schema, exact identities, all four Local Ignore states, and structured diagnostics without raw documents. Entry points are CXX `installed_yaml_data_load(...)` plus its status/snapshot/recovery-plan/reset functions, Node `loadInstalledYamlData(...)`, and Python `classic_config.load_installed_yaml_data(...)`.

**Crash Log Scan Run Installed YAML Data contract (`classic-scanlog-core`):** the final Rust-owned run accepts one installation root and typed game, performs one Installed YAML Data load after discovery, and retains that immutable snapshot through analysis. Existing or generated Ignore proceeds immediately. Malformed Ignore returns the distinct Local Ignore Recovery Required lifecycle state with completed discovery, structured diagnostics, and a Rust-owned opaque single-use continuation. CXX, Node, and Python explicitly pass Proceed Without Ignore or Reset To Default to resume the same prepared intake in discovery order; no adapter rediscovers inputs, reselects Main/game data, or serializes/clones the carrier. Reset uses the retained config recovery plan, returns durable byte-exact backup and replacement metadata on success, and preserves conflict/backup/replacement failure plus durability-unknown recovery receipts as stable typed resume outcomes. Pre-reset cancellation performs no filesystem or analysis work. Once reset begins, it completes its non-interruptible durable transaction before cancellation is observed and the normal cancelled-after-discovery result is returned without analysis. Replay rejects with `scan_run_continuation_consumed`. The public cross-adapter pack proves retained identities and discovery, success, intervening conflict, portable backup failure, byte-exact backup, report-byte stability, pre-discovery, post-discovery queued, admitted/durable, pre-reset, and post-critical cancellation boundaries, structured observer delivery failure, and replay. It also proves public request-validation, discovery, intake, report-write, and Unsolved Logs finalization failures with stable stage, nonempty-message contract, relevant path, per-log disposition, and artifact type. Replacement-publication, durability-unknown, injected analysis, FormID database access, initialization, and internal-invariant mappings have no safe deterministic public trigger, so named blocking internal fault analyzers preserve their Rust and binding projections without claiming semantic adapter receipts.

**Crash Log Scan Run Display Content (`classic-scan-presentation`):** this crate has no binding row of its own, because it is never called across a seam. A Crash Log Scan Run Result retains a one-shot continuation and is not clonable, so a binding cannot hold one and render from it later; instead each binding renders while the Rust value is live and carries the result across as mirrored data. On CXX that means `ScanRunContractExecutionResult.display_lines` and `ScanRunContractEvent.display_lines`, both in [`scanner.rs`](../../cpp-bindings/classic-cpp-bridge/src/scanner.rs), with each segment flattened to a `ScanRunDisplaySegmentKind` tag plus text, path, and count fields because CXX cannot express a payload-carrying Rust enum. Event lines are populated inline on the observer callback, before the event reaches the consumer. The six segment kinds are frozen: adding one costs a baseline regeneration on all three binding surfaces. All four native frontends render from it, and Node and Python mirror the same flattening. Full shape and adapter rules: [`classic-cpp-bridge-data-entrypoints.md`](classic-cpp-bridge-data-entrypoints.md) and [`classic-scan-presentation.md`](classic-scan-presentation.md).

CLI, GUI, and TUI emit separate blocking consumer receipts for their maintained Display Content, routing, interaction, and presentation obligations. These receipts preserve typed path/count carriers and are validated against source-owned obligation profiles; they cannot satisfy semantic adapter parity rows or authorize frontend recomposition of Rust-owned wording. Diagnostic artifacts upload even when a retained or replacement gate fails.

The run's one interactive surface crosses the same way. A Local Ignore recovery prompt is rendered while the Rust value is live and carried as `ScanRunRecoveryPrompt` on CXX (behind `has_recovery_prompt`, since CXX has no optional struct), `JsScanRunSuccess.recoveryPrompt` on Node, and `ScanRunExecution.recovery_prompt` on Python — present only for a run whose status is Local Ignore Recovery Required. Each `RecoveryDecisionDescription` carries the decision, its Display Label, a description flattened exactly as any line, and **`available`**. That last field travelling *on the decision* rather than beside the prompt is the contract: a consumer must not offer a decision marked unavailable, and it cannot fail to see the fact without ignoring data placed directly in its hands. This closed a confirmed gap in which `local_ignore_reset_available` sat one level away on the Installed YAML Data DTO and neither C++ frontend read it, so choosing Reset To Default spent the one-shot continuation on a guaranteed failure. `LocalIgnoreRecoveryDecision` still has exactly two variants on all five surfaces; backing out is the absence of a decision, through the abandon entry point.

**Removed YAML Data selection bypasses (all bindings):** the four contracts above are now the *only* ways to obtain YAML Data. The positional two/three-directory loader (`YamlDataCore::load_from_yaml_files`) and its per-binding constructors — CXX `yaml_data_load(...)`, Node `new YamlData(yamlDirs, ...)`, and Python `YamlData(yaml_dirs, ...)` / `create_yamldata(...)` — are deleted, as is the path-backed Crash Log Scan Intake source. `classic-config-core`'s low-level shippable selection and its caller-supplied compatibility range are crate-private. A `YamlData` / `YamlDataCore` value is therefore reachable only by consuming an Installed or Explicit snapshot, or through the path-free `from_yaml_content` content-string factory retained for tests. The compliance suite's `forbiddenExports` audit asserts each removed name stays absent from its owning surface, and the C++, Node, and Python parity baselines were refreshed to match.
...
The Node surface in [`classic-node`](../../node-bindings/classic-node/src/lib.rs) uses `#[napi(object)]` structs for DTOs, opaque native classes for invariant-preserving values, and async Rust functions that map naturally to JavaScript promises. NAPI-RS auto-converts `snake_case` Rust identifiers to `camelCase` at the JS boundary. User Settings exposes separate ordinary-update and explicit-bootstrap preview/commit pairs; frontends project an accepted snapshot into the opaque final Crash Log Scan Run request; and Game Setup has both explicit-facts and explicit-root typed entry points. The flat `ClassicConfigJs`/`JsPathConfig`/`createDefaultConfig` surface is removed across all layers. Crash Log Scan Run envelopes, rejections, and observed events carry `displayLines` — Rust-owned Display Content flattened exactly as the C++ bridge flattens it, so a Node frontend speaks with CLASSIC's voice without reimplementing its wording — and a paused run's success envelope carries `recoveryPrompt`, whose decisions each carry their own `available`. The committed [`index.d.ts`](../../node-bindings/classic-node/index.d.ts) is the tracked generated contract artifact.

See: [`node-python-contract-map.md`](node-python-contract-map.md).

### Python (PyO3)

The Python surface is a set of per-crate PyO3 modules under [`python-bindings/`](../../python-bindings/). Each module uses `#[pyclass]` wrappers with `#[getter]` properties and `#[pyo3(name="...")]` for Python-convention naming. The intentional breaking User Settings cutover removed `ClassicConfig` and `PathConfig`; inspection, updates, conflicts, and migrations use the explicit-root `classic_user_settings` contract. Crash Log Scan Runs now use the same final Rust-owned contract as C++ and Node through invariant-preserving `ScanRunRequest` factories, separate opaque cancellation, complete serialized events, typed result/error mapping, and adapter-only observer failure data. The execution envelope, every resume exception, and every observed event carry `display_lines` — Rust-owned Display Content flattened exactly as the C++ bridge flattens it, so a Python tool speaks with CLASSIC's voice without reimplementing its wording — and a paused run's envelope carries `recovery_prompt`, whose decisions each carry their own `available`.
