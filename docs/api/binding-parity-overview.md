...
**Installed YAML Data load contract (`classic-config-core`):** Rust, C++, Node, and Python load one installation root, typed game, and separate selected-game-version mode into a typed Ready outcome when valid Local Ignore YAML Data already exists. Every surface exposes parsed YAML Data, independently selected Main/game provenance and schema, exact identities for all three retained files, `Existing` Local Ignore state, and structured fallback diagnostics without exposing raw bytes or YAML documents. Existing Local Ignore content is never replaced during ordinary loading. Entry points are CXX `installed_yaml_data_load(...)` plus its status/snapshot functions, Node `loadInstalledYamlData(...)`, and Python `classic_config.load_installed_yaml_data(...)`; every fatal core variant remains typed on each surface.
