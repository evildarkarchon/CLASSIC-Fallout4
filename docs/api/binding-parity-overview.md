...
**Installed YAML Data load contract (`classic-config-core`):** Rust, C++, Node, and Python load one installation root, typed game, and separate selected-game-version mode into a typed Ready outcome when Local Ignore YAML Data is valid or can be generated. Missing Local Ignore is strictly validated from the retained selected Main defaults, atomically published without clobbering a concurrent winner, and reread from the authoritative canonical file. Every surface exposes parsed YAML Data, independently selected Main/game provenance and schema, exact identities for all three retained files, `Existing` or `Generated` Local Ignore state, and structured fallback/`LocalIgnoreGenerated` diagnostics without exposing raw bytes or YAML documents. Existing Local Ignore content is never replaced during ordinary loading, and default-validation, creation, or reread failures remain typed. Entry points are CXX `installed_yaml_data_load(...)` plus its status/snapshot functions, Node `loadInstalledYamlData(...)`, and Python `classic_config.load_installed_yaml_data(...)`.
