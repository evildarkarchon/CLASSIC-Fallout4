...
**Crash Log Scan Run Installed YAML Data contract (`classic-scanlog-core`):** the final Rust-owned run accepts one installation root and typed game, performs one Installed YAML Data load after discovery, and retains that immutable ready snapshot through analysis. CXX, Node, and Python result projections expose optional selected Main/game facts, Local Ignore state/identity, and structured diagnostics when intake was reached. Adapters neither reopen selected paths nor render these diagnostics into Autoscan Reports. The shared valid/generated fixture proves identical report bytes while metadata records whether Local Ignore existed or was generated.
