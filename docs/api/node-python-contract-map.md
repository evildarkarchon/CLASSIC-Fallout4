...
- Installed YAML Data loading lives in `classic_config.load_installed_yaml_data(installation_root, game, selected_game_version)`. Its return type is the union of frozen `InstalledYamlDataLoadOutcome` (Ready with a non-optional snapshot) and `InstalledYamlDataLocalIgnoreRecoveryRequiredOutcome` (a non-optional opaque `LocalIgnoreRecoveryPlan`). Ready snapshots expose parsed `yaml_data`, Main/game facts, exact Local Ignore identity, `existing` or `generated` state, and fallback/`local_ignore_generated` diagnostics. The plan exposes retained metadata/identities/diagnostics and a single-use `proceed_without_ignore()` method; the returned snapshot has `proceed_without_ignore` state, an empty operation-scoped list, and no filesystem mutation. Six genuinely fatal core variants retain dedicated `InstalledYamlDataLoadError` subclasses with stable `code`, optional `yaml_role`, optional `path`, and structured diagnostics
