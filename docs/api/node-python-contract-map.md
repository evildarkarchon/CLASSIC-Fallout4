...
- complete Crash Log Scan Runs start with `scanRunExecute(request, cancellation, observer?, cancelOnObserverError?) -> Promise<JsScanRunSuccess | JsScanRunFailure>`, authored in [`src/scan_run.rs`](../../node-bindings/classic-node/src/scan_run.rs). A `local_ignore_recovery_required` result owns an opaque `ScanRunContinuation`; `scanRunResume(continuation, JsScanRunLocalIgnoreRecoveryDecision.ProceedWithoutIgnore, cancellation, observer?, cancelOnObserverError?)` resumes the retained discovery/snapshot once and rejects replay with `error.code === "scan_run_continuation_consumed"`. Node exports no orchestration, analysis-only execution, batch lifecycle, direct report-writing, or global-FCX alternative
...
- complete Crash Log Scan Runs start through `ScanRunRequest` factories plus `scan_run_execute(request, cancellation, observer=None, cancel_on_observer_error=False) -> ScanRunExecution`. A `local_ignore_recovery_required` result owns an opaque `ScanRunContinuation`; `scan_run_resume(continuation, ScanRunLocalIgnoreRecoveryDecision.ProceedWithoutIgnore, cancellation, observer=None, cancel_on_observer_error=False)` resumes retained work once, releases the GIL during execution, and raises `ScanRunContinuationConsumedError` with code `scan_run_continuation_consumed` on replay. Python exports no orchestration, analysis-only execution, resettable batch token, direct report-writing, or global-FCX alternative
...
- successful scan results expose optional `installed_yaml_data` with selected Main/game role, provenance, schema and exact-byte identity, `existing`, `generated`, `recovery_required`, or `proceed_without_ignore` Local Ignore state/identity, and structured fallback, generation, or malformed-file diagnostics. A recovery-required result also exposes one opaque process-local `ScanRunContinuation`; `scan_run_resume(..., ProceedWithoutIgnore, ...)` reuses retained discovery, prepared intake, and the exact snapshot without changing malformed bytes
