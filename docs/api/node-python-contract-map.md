...
- full Crash Log Scan Run adapter work uses `scanRunExecute(request, cancellation, observer?, cancelOnObserverError?) -> Promise<JsScanRunSuccess | JsScanRunFailure>`, authored in [`src/scan_run.rs`](../../node-bindings/classic-node/src/scan_run.rs); lower-level `processLog*` and `processLogsBatch*` functions remain analysis/report-line entry points for callers that explicitly need that shape
...
- lower-level FCX analysis takes `classicRoot?: string` through `JsAnalysisConfig` or `JsAnalysisBuildOptions` and rejects FCX preparation when that explicit User Settings root is absent
