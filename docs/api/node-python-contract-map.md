...
- User Settings inspection uses `openUserSettings(classicRoot) -> JsUserSettingsSnapshot`, authored in [`src/user_settings.rs`](../../node-bindings/classic-node/src/user_settings.rs); it exposes typed Update Preferences, Crash Log Scan settings, and Game Setup settings plus source/schema/revision metadata, diagnostics, commit eligibility, and retained source bytes
