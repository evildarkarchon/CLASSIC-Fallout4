...
- `runGameSetupIntake(options)` returns ordered `pathUpdates: JsGameSetupPathUpdate[]` alongside `pathUpdateCount`, so a caller can review and explicitly map selected proposals into a later User Settings Update; UTF-8-representable paths are exact, while other native paths use replacement characters at the JavaScript string boundary
