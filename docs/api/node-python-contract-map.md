...
- full Crash Log Scan Run adapter work should use `scanRunExecute(logPaths, options) -> Promise<JsScanRunResult>`; lower-level `processLog*` and `processLogsBatch*` functions remain analysis/report-line entry points for callers that explicitly need that shape
...
- both Game Setup entry points return ordered `pathUpdates: JsGameSetupPathUpdate[]` alongside `pathUpdateCount`, so a caller can review and explicitly map selected proposals into a later User Settings Update; UTF-8-representable paths are exact, while other native paths use replacement characters at the JavaScript string boundary
...
- omitted `scanRunExecute` scan flags, custom input, concurrency, Unsolved Logs policy, and FCX setup facts come from typed User Settings under `yamlDirRoot`; explicit option values override the saved values
...
- `classic_scangame.GameSetupIntake` accepts typed game/executable/documents/mods/custom-scan/Papyrus paths, and `run_game_setup_intake(...)` returns concrete `GameSetupPathUpdate` records alongside `path_update_count`; the caller decides whether any proposal becomes a User Settings Update
...
- `max_concurrent=0` means the core adaptive default (equivalent to `None`), matching the Node and C++ surfaces
