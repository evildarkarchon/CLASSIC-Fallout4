...
- successful scan results expose optional `installed_yaml_data` with selected Main/game role, provenance, schema and exact-byte identity, `existing`/`generated` Local Ignore state/identity, and structured fallback or generation diagnostics. These #146 run-level diagnostics never enter Autoscan Report text; malformed-Ignore continuation/resume states remain reserved for #147
...
Node and Python public-seam tests consume the same immutable corpus and normalized expectations from [`tests/fixtures/crash_log_scan_run/manifest.json`](../../tests/fixtures/crash_log_scan_run/manifest.json) as Rust and CXX. That corpus includes valid existing and generated Local Ignore runs and pins their stable Installed YAML Data metadata separately from byte-identical Autoscan Reports. The canonical binding compliance profile compares the manifest's request, event, status, discovery, disposition, failure, infrastructure, movement, progress, cancellation, Installed YAML Data role/provenance/diagnostic, and Local Ignore state variants with the Rust inventory and requires explicit acknowledgement from both language adapters.
