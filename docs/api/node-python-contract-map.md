...
- Installed YAML Data loading uses `loadInstalledYamlData({ installationRoot, game, selectedGameVersion }) -> Promise<JsInstalledYamlDataLoadOutcome>` from the same module. Ready outcomes own an immutable `InstalledYamlDataSnapshot` exposing parsed `yamlData`, Main/game facts, exact authoritative Local Ignore identity, `Existing` or `Generated` state, and fallback/`LocalIgnoreGenerated` diagnostics. Missing Local Ignore is initialized by config core from validated selected-Main defaults with no-clobber publication; all nine fatal core variants reject with stable lowercase `code` plus applicable `yamlRole`, `path`, and `diagnostics` metadata
...
- Installed YAML Data loading lives in `classic_config.load_installed_yaml_data(installation_root, game, selected_game_version)`. It returns a frozen Ready outcome and snapshot with parsed `yaml_data`, Main/game facts, exact authoritative Local Ignore identity, `existing` or `generated` state, and fallback/`local_ignore_generated` diagnostics. Missing Local Ignore is initialized by config core from validated selected-Main defaults with no-clobber publication; each of the nine fatal core variants has a dedicated `InstalledYamlDataLoadError` subclass with stable `code`, optional `yaml_role`, optional `path`, and structured diagnostics
