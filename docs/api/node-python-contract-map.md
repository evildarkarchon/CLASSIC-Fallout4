...
- Installed YAML Data loading lives in `classic_config.load_installed_yaml_data(installation_root, game, selected_game_version)`. It returns a frozen Ready outcome and snapshot with parsed `yaml_data`, Main/game facts, exact Local Ignore identity, `existing` state, and diagnostics. Each fatal core variant has a dedicated `InstalledYamlDataLoadError` subclass with stable `code`, optional `yaml_role`, optional `path`, and structured diagnostics
