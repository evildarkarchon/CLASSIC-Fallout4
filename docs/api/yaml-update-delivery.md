...
- Native C++ bridge callers use `yaml_data_check_update(enabled)`, `yaml_data_apply_update(enabled, approved)`, and `yaml_data_rollback_update()`. For check/apply, the Rust first-party helper discovers the installation root from the union of native frontend candidates (executable directory, CWD, executable parent/grandparent, and the supported `install` children) and requires the selected root to contain `CLASSIC Data`. The bridge returns the same status/apply DTO shapes as the generic functions; rollback returns an aggregate `YamlRollbackReportDto`. Typed apply errors map to stable `error_message` prefixes: `"update check disabled: ..."` and `"decision stale: ..."`. GUI / CLI consumers parse the prefix.
...
- Node exposes the first-party operations only: `checkYamlDataUpdate(enabled, installationRoot?)`, `applyYamlDataUpdateWithDecision(enabled, approved, installationRoot?)`, and `rollbackYamlDataUpdate()`, where `approved` is `JsApprovedUpdate { releaseTag, fileNames, fileSha256 }`. Apply throws on disabled / stale. There is deliberately no generic Node variant accepting caller-supplied channel coordinates or schema entries — an adapter able to declare its own accepted ranges could classify, and then install, under a policy `classic-config-core` does not own. Node/Bun callers pass their package-local `installationRoot` because the native `current_exe()` fallback resolves to the `node.exe` / `bun.exe` tree rather than the CLASSIC install. `rollbackYamlDataUpdate()` returns `JsYamlRollbackTargetOutcome[]` in Rust-expanded target order; a per-target failure is reported as a non-null `errorMessage` on that entry rather than rejecting the whole promise.
...
First-party checks identify one CLASSIC installation root. The binding-compatible `UpdateCheckConfig::bundled_yaml_dir` layout hint accepts either that root or its canonical `CLASSIC Data/databases` directory. When native frontends omit the hint, the first-party helper selects the first root containing `CLASSIC Data` from the shared native layout candidates: executable directory, CWD, executable parent, executable grandparent, `<executable-parent>/install`, then `<CWD>/install`. Config inspection resolves bundled candidates only beneath `<installation_root>/CLASSIC Data/databases` and resolves the per-user update cache through `classic-path-core`.
