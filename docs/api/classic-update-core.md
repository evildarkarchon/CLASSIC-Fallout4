...
Error variants for notification failures live on `UpdateError` as `NotificationFetchFailed`, `NotificationDecode`, `NotificationInstalledVersionParse`, and `NotificationCacheIo` (design D-05 — nested on `UpdateError` rather than a sibling enum so the per-binding error mapping does not double). See [`error-contract.md`](error-contract.md) for per-binding shapes.
