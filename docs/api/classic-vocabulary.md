...
`InfrastructureErrorStage` is in the TUI's audited set without being adopted. The TUI renders that stage's Vocabulary *Token* through `Display`, inside a message the contract composes, so switching it to the label would change shipped output and belongs to whichever ticket decides that wording. Listing it costs nothing and means the switch, when it happens, cannot be made by writing a table.
