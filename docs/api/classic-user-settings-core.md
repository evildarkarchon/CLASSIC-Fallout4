...
`AcceptedUserSettingsUpdate::commit(classic_root)` first checks the current exact-byte SHA-256 `Revision` against the preview revision. An already-stale request returns `UserSettingsCommitOutcome::Conflict { expected_revision, actual_revision }` without creating a lock file or changing the newer document. Otherwise it acquires the exclusive cross-process lock on the persistent `CLASSIC Settings.yaml.commit.lock` sibling, reopens the canonical document, and repeats the revision check before any patch or publication. This second check protects against an edit arriving after the preflight; a conflict at this boundary may retain the coordination lock. A matching revision patches only the accepted canonical fields and returns `Committed { revision }`; rejected previews never produce an accepted artifact and therefore cannot call this operation. The early no-write conflict behavior matches the stateless Node and CXX commit seams.
...
- Node: `openUserSettings(classicRoot)` returns `JsUserSettingsSnapshot` with all four typed groups and exact source bytes. `JsUserSettingsUpdate` accepts optional typed `windowGeometry` and `tui` transitions; preview/commit and bootstrap calls remain read-only then explicit. `importLegacyTuiStateIntoUserSettings(classicRoot, legacyStatePath)` returns structured status, conflict, revision, backup, and opaque receipt data; `receipt.restore(classicRoot)` performs conflict-safe restoration. Node frontends project an accepted snapshot into the explicit `ScanRunRequest` configuration, source, setup context, and Standard-only movement policy before calling `scanRunExecute(...)`; the scan adapter does not reopen User Settings. For FCX-enabled requests, the run-scoped setup context carries the explicit CLASSIC root and Rust performs setup inside that final operation. `runGameSetupIntakeFromUserSettings(classicRoot, xseLogPath?)` remains the cohesive read-only Game Setup path. The intentional breaking cutover removed `ClassicConfigJs`, `JsPathConfig`, and `createDefaultConfig`. Operational failures retain stable core error codes on the thrown Node error.
- Python: `classic_user_settings.open_user_settings(classic_root)` returns `UserSettingsSnapshot` with all four typed groups and exact source bytes. Preview, explicit commit, conflict, migration apply, and receipt restore remain typed Python objects. Callers project one accepted snapshot into `ScanRunConfiguration`, a typed Standard or Targeted source, optional `ScanRunSetupContext`, and the Standard-only movement policy before calling `classic_scanlog.scan_run_execute(...)`; the scan adapter never reopens User Settings. For FCX-enabled requests, that setup context carries the explicit CLASSIC root and the final operation consumes the same typed groups without interpreting flat keys. The intentional breaking cutover removed Python `ClassicConfig` and `PathConfig` exports.
