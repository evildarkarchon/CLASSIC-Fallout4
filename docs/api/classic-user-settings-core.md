...
The shared module's stage vocabulary ends in `Publish`, while this crate's ends in `Replace`. The projection is deliberate and preserves every code below unchanged, including `commit_replace_failed` and the per-operation `*_replace_failed` codes derived from it.
