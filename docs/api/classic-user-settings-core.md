...
`UserSettingsUpdate` is an explicit request builder covering Update Check, Update Source, Auto Switch After Scan, and every field in `CrashLogScanSettings` and `GameSetupSettings`. `UserSettings::preview_update(update)` performs no I/O and returns exactly one of:
...
An accepted preview is anchored to the opened `Revision` and contains only the requested canonical `UserSettingsUpdateField` values. `canonical_path()` reports names such as `/CLASSIC_Settings/Update Source`, `/UI/preferences/auto_switch_after_scan`, and `/CLASSIC_Settings/Max Concurrent Scans`. Existing aliases, unknown entries, unrelated known-invalid values, and non-requested settings are never added as repair or normalization work.
...
Preview-specific rejection codes include `invalid_enum_update_source`, `invalid_enum_game_version`, the `invalid_path_*` codes for every optional setup path, `invalid_value_formid_databases`, `invalid_path_unsolved_logs_destination`, `invalid_path_custom_scan_input`, `invalid_range_max_concurrent_scans`, `update_base_requires_bootstrap`, `bootstrap_base_not_missing`, and `update_base_not_commit_eligible`.
...
- CXX: `classic::settings::user_settings_open_update_preferences(classic_root)` retains the narrow update-policy DTO; `user_settings_open_crash_log_scan_settings(classic_root)`, `user_settings_open_game_setup_settings(classic_root)`, and `user_settings_open_frontend_state(classic_root)` expose cohesive consumer groups. `user_settings_open_gui_settings(classic_root)` provides the revision-cohesive native-GUI aggregate, while `user_settings_gui_published_defaults()` provides the same shape from the Rust registry without filesystem access. `user_settings_preview_update(classic_root, update)` plus `user_settings_commit_update(...)` handle ordinary updates, while `user_settings_preview_bootstrap(classic_root, update)` plus `user_settings_commit_bootstrap(...)` are the only missing-document path. Both commit operations revalidate through the locked Rust core. Migration persistence uses `user_settings_apply_migration(...)`, the opaque `UserSettingsMigrationApplyHandle`, `user_settings_migration_apply_outcome(...)`, and `user_settings_restore_migration(...)`.
- Node: `openUserSettings(classicRoot)` returns `JsUserSettingsSnapshot` with all four typed groups, including the canonical `updatePreferences.updateSource` token and provenance, plus exact source bytes as `originalContent`; `publishedUserSettingsDefaults()` returns the same snapshot shape from the Rust registry without a root path or filesystem access. `JsUserSettingsUpdate` accepts `updateSource` and `autoSwitchAfterScan` alongside the existing fields. `previewUserSettingsUpdate(classicRoot, update)` plus `commitUserSettingsUpdate(...)` handle ordinary updates, while `previewUserSettingsBootstrap(classicRoot, update)` plus `commitUserSettingsBootstrap(...)` are the explicit missing-document seam. Both previews are read-only, both commits return committed, conflict, or rejected outcomes, and operational failures throw stable coded errors. `applyUserSettingsMigration(...)` returns an opaque `JsUserSettingsMigrationReceipt` on success, and `receipt.restore(...)` performs explicit restoration.
- Python: `classic_user_settings.open_user_settings(classic_root)` returns `UserSettingsSnapshot` with all four typed groups and exact source bytes, while `user_settings_published_defaults()` returns the same snapshot shape from the Rust registry without a root path or filesystem access. `snapshot.preview_update(UserSettingsUpdate)` validates ordinary updates against that opened snapshot and revision without writing. Missing documents are accepted only through `snapshot.preview_bootstrap(UserSettingsUpdate)`, whose retained accepted artifact publishes the complete Rust-owned default registry when the caller explicitly invokes `commit(classic_root)`. `UserSettingsMigrationPlan.apply(...)` retains the opaque core receipt in `UserSettingsMigrationReceipt`, whose `restore(...)` method performs explicit restoration.
