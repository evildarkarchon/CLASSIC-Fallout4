...
- A clobbering replacement and a unique-path publication are distinct. `publish` replaces; the backup half of `publish_with_verified_backup` refuses to overwrite and surfaces an existing path as a `Publish`-stage failure with an `AlreadyExists` source.
...
- `LockPolicy::at(path)` locks exactly the path the caller derived. This exists because `classic-config-core`'s Local Ignore reset lock is a fixed name *two directories above* the target, which a sibling suffix cannot express; without it, migrating that caller would have to move a lock file on disk.
