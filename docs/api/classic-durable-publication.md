...
- `LockPolicy::acquire` is public so that a caller's *neighbouring* operations — ones that publish nothing but move the same files an install moves — can serialize against it on the same lock file. `classic-file-io-core`'s `rollback` and `self_heal` are the only such callers: they swap or promote `<target>.prev` and must not interleave with an install's rotation. Using this rather than a second hand-rolled lock is what keeps the workspace at one cross-process lock implementation; it is also what let `classic-file-io-core` drop its `fs4` dependency entirely.
