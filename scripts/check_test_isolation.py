...
class TestIsolationChecker:
    ...
    def _is_safe_yaml_usage(self, line: str, prev_lines: list[str]) -> bool:
        ...
        for prev_line in prev_lines[-3:]:  # Check last 3 lines
            if any(pattern in prev_line for pattern in [
                '@patch', 'with patch', '@mock', 'with mock'
            ]):
                return True
    ...
    def _is_safe_path_usage(self, line: str, prev_lines: list[str]) -> bool:
        ...
        for prev_line in prev_lines[-3:]:
            if any(pattern in prev_line for pattern in [
                '@patch', 'with patch', 'mock_path'
            ]):
                return True

        return False

    def _is_safe_write_operation(self, line: str, file_lines: list[str],
                                 line_num: int) -> bool:
        """
        ...
        for i in range(max(0, line_num - 30), line_num):
            ...
            for tracked_var in derived_from:
                if tracked_var in check_line and any(pattern in check_line for pattern in [
                    'tmp_path /', 'tmp_path/', 'self.tmp_path', 'tmp_path\\'
                ]):
                    return True
