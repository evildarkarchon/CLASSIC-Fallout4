...
export function observeAuxOperations(family: string, fixture: JsonObject): JsonObject {
  ...
  if (family === "web-operations") {
    ...
    return {
      ...
      query: result(() => classic.buildUrlWithQuery(url, request.params.map(([key, value]: string[]) => ({ key, value })))),
    };
  }
  if (family === "resource-operations") {
    ...
    try {
      ...
      observation.resources = classic.enumerateResources(root).map(item => ({ path: relative(root, item.path).replaceAll("\\", "/"), type: item.resourceType, size: item.size })).sort((a, b) => a.path.localeCompare(b.path));
      observation.counts = classic.countResourcesByType(root).map(item => ({ type: item.resourceType, count: item.count })).sort((a, b) => a.type.localeCompare(b.type));
      ...
      observation.validation = request.validate.map((path: string) => {
        let error = null;
        try { classic.validateResource(owned(root, path)); }
        catch (failure) {
...
function resourceFiles(root: string, directory: string): { path: string; hex: string }[] {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile()) files.push({ path: relative(root, path).replaceAll("\\", "/"), hex: readFileSync(path).toString("hex") });
