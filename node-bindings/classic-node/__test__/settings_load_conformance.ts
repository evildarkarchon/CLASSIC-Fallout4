...
function domainError(error: unknown, root: string, paths: string[]): JsonObject {
    ...
    for (const [prefix, kind] of [["Failed to read file ", "io"], ["Failed to parse YAML from ", "yaml-parse"]]) {
        for (const relative of paths) if (message.startsWith(`${prefix}${owned(root, relative)}: `)) return {
            kind,
            path: relative
        };
    }
...
export async function observeSettingsLoad(fixture: JsonObject): Promise<JsonObject> {
    ...
    try {
        for (const [relative, content] of Object.entries(fixture.files)) {
            const path = owned(root, relative);
        ...
        for (const operation of ["sync", "async", "batchSync", "batchAsync"]) {
            ...
            try {
                ...
                error = domainError(failure, root, relatives);
            }
            ...
            classic.resetSettingsCacheStats();
            cacheState.resetStats = cacheStatistics();
            // Refill entries so clear is checked independently of invalidation.
            keys.forEach((key, index) => {
                if (cached[index]) classic.loadSettingsSync(key, paths[index]);
            });
            classic.clearSettingsCache();
            observed[operation] = {
                count,
                ...
                cached,
                cacheState,
                afterClear: keys.map(key => classic.isCached(key))
            };
        }
...
async function observeSettingsYaml(fixture: JsonObject): Promise<JsonObject> {
    ...
    try {
        ...
        const before = {
            name: classic.yamlGetStringValue(fixture.content, "name", "fallback"),
            missing: classic.yamlGetStringValue(fixture.content, "absent", "fallback"),
            items: classic.yamlGetVecValue(fixture.content, "items"),
            mapping: classic.yamlGetHashmapValue(fixture.content, "mapping")
        };
        ...
        const loaded = classic.yamlLoadFile(path);
        ...
        return {
            ...
            cache: {
                ...
                size: stats.size,
                afterClear: classic.yamlGetCacheStats().size
            }
        };
    } finally {
