...
export function observeVersionExtended(fixture: Record<string, any>): Record<string, any> {
    ...
    if (request.operation === "extract") return {
        filename: classic.extractVersionFromFilename(request.filename),
        log: classic.extractVersionFromLog(request.content),
        all: classic.extractAllVersions(request.content)
    };
