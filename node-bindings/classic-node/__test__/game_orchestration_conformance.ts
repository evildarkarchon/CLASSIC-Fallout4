...
export async function observeOrchestration(fixture: any): Promise<Record<string, unknown>> {
    ...
    try {
        for (const [name, content] of Object.entries(fixture.files)) {
            if (name !== "sentinel.txt") throw new Error("unsupported orchestration fixture path");
            await writeFile(join(root, name), String(content));
        }
        const config = {
            gamePath: root,
            xseAcronym: fixture.xseAcronym,
            crashgenName: fixture.crashgenName,
            gameName: fixture.gameName,
            xseScriptfiles: {},
            isVr: false,
            gameVersion: "Original",
            wryeWarnings: {},
            logCatchErrors: ["error"],
            logExcludeFiles: [],
            logExcludeErrors: [],
            gameTarget: "Fallout4"
        };
        const result = await classic.runGameChecks(config);
        const mods = await classic.runModScans(config);
        ...
        const game = {
            checkResults: [...result.checkResults].sort((a, b) => a.name.localeCompare(b.name)),
            reportMatchesChecks,
            configIssueCount: result.configIssues.length,
        ...
        return {
            ...
            mods: {
                report: mods.report,
                unpackedIssueCount: mods.unpackedIssueCount,
                archivedIssueCount: mods.archivedIssueCount,
                errors: [...mods.errors].sort()
            },
            files
        };
    } finally {
