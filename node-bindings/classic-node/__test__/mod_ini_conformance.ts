...
import { scanModInis,JsConfigDuplicateDetector,detectConfigDuplicates } from "../index.js";
...
export async function observeModIni(fixture: Record<string, any>): Promise<Record<string, any>> {
  ...
  try {
    ...
    if(fixture.operation==="duplicates"){
      const groups=(values:Record<string,any>[])=>values.map(group=>({original:normalized(group.original),duplicates:group.duplicates.map(normalized).sort()})).sort((left,right)=>left.original<right.original?-1:left.original>right.original?1:0);
      ...
      const detector=new JsConfigDuplicateDetector();const initialGroups=groups(detector.detectDuplicates(root));const initialMap=mapping(detector.getDuplicateMap(root));
      ...
      const path=fixture.replacement.path;if(path.includes(":")||path.includes("\\")||path.split("/").some((part:string)=>["", ".", ".."].includes(part)))throw new Error("invalid replacement path");
      await writeFile(join(root,path),fixture.replacement.content,"utf8");
      return {operation:"duplicates",before,result:{initialGroups,initialMap,afterGroups:groups(detector.detectDuplicates(root)),afterMap:mapping(detector.getDuplicateMap(root))},files:await inventory(root)};
    }
    const value=scanModInis(root,fixture.game);
    return {operation:"scan",before,result:{message:value.message.replaceAll(root,"<ROOT>").replaceAll("\\","/"),
      ...
      duplicates:value.duplicates.map(entry=>({name:entry.fileName,paths:entry.paths.map(normalized).sort()}))},files:await inventory(root)};
