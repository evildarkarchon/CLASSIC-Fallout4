...
export async function observeDatabaseOperations(fixture: JsonObject): Promise<JsonObject> {
    if (Object.keys(fixture).join() === "operation" && fixture.operation === "cache-defaults") {
        const result = {
            defaultTtl: classic.getDefaultCacheTtl(),
            batchTtl: classic.getBatchCacheTtl(),
            maximumTtl: classic.getMaxCacheTtl(),
            capacity: classic.getDefaultQueryCacheCapacity(),
            cleanupThreshold: classic.getDefaultCacheCleanupThreshold(),
            cleanupInterval: classic.getDefaultCacheCleanupInterval()
        };
        const constants = {
            defaultTtl: classic.DEFAULT_CACHE_TTL,
            batchTtl: classic.BATCH_CACHE_TTL,
            maximumTtl: classic.MAX_CACHE_TTL,
            capacity: classic.DEFAULT_QUERY_CACHE_CAPACITY,
            cleanupThreshold: classic.DEFAULT_CACHE_CLEANUP_THRESHOLD,
            cleanupInterval: classic.DEFAULT_CACHE_CLEANUP_INTERVAL
        };
    ...
    try {
        ...
        const result: JsonObject = {
            table: pool.getGameTable(),
            initialAvailable: pool.isAvailable(),
            error: null,
            single: [],
            batch: []
        };
        try {
            try {
                await pool.initialize([join(root, "formids.db")]);
            } catch (failure: any) {
