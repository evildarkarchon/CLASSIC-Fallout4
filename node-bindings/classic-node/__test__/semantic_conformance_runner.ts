...
function crashgenRequest(request: JsonObject): classic.JsCrashgenSettingsAnalysisInput {
    return {
        settings: Object.entries(object(request.settings, "settings")).flatMap(([section, values]) =>
            Object.entries(object(values, "section settings")).map(([key, value]) => ({
                section,
                key,
...
async function analyze(family: string, fixture: JsonObject): Promise<JsonObject> {
    ...
    try {
        if (family === "formid-lookup") {
            ...
            switch (config.mode) {
                case "disabled":
                    lookup = classic.JsFormIdValueLookup.disabled();
                    break;
                case "in-memory":
                    lookup = classic.JsFormIdValueLookup.inMemory(config.entries.map((entry: JsonObject) => ({
                        formid: entry.formid, plugin: entry.plugin, value: entry.value ?? undefined,
                        operationalFailure: entry.operationalFailure ?? undefined,
                    })));
                    break;
                case "sqlite-missing":
                    lookup = classic.JsFormIdValueLookup.sqlite(config.databasePath, config.gameTable);
                    break;
                case "shared-pool":
                    lookup = classic.JsFormIdValueLookup.fromSharedPool(new classic.JsDatabasePool(config.gameTable));
                    break;
                default:
            ...
            if ("pairs" in request) {
                ...
                return {
                    analyzerKind: null,
                    result: {outcomes: results.map((result) => fields(result, ["kind", "value"]))},
                    error: null
                };
            }
        ...
        switch (family) {
            ...
            case "plugin-evidence":
                ...
                break;
            default:
                throw new Error("unsupported semantic family");
        }
        ...
        return {
            analyzerKind, result: null, error: {
...
async function buildReceipt(plan: JsonObject): Promise<JsonObject> {
    ...
    return {
        ...
        runner: {
            id: "classic-node-semantic-conformance",
            version: 1,
