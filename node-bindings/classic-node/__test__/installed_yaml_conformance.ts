...
function selectedFile(value: classic.JsInspectedYamlDataFile): JsonObject {
    ...
    return {
        role,
        provenance,
        schemaVersion: `${value.schemaMajor}.${value.schemaMinor}`,
        identity: identity(value, path)
    };
}
...
function diagnostics(root: string, values: classic.JsInstalledYamlDataDiagnostic[]): JsonObject[] {
    return values.map((item) => ({
        role: token(item.role),
        candidate: token(item.candidate),
        path: pathCarrier(root, item.path),
...
export async function observeInstalledYaml(fixture: JsonObject): Promise<JsonObject> {
    ...
    try {
        ...
        const request = {
            ...
            selectedGameVersion: fixture.selectedGameVersion
        };
        const observation: JsonObject = {
            outcome: "error", game: null, gameDataRole: null, main: null, gameFile: null,
        ...
        try {
            ...
            const native = error as Error & {
                code?: string;
                yamlRole?: string;
                diagnostics?: classic.JsInstalledYamlDataDiagnostic[]
            };
            ...
            observation.error = {
                code: native.code,
                role: ["main", "game"].includes(native.yamlRole ?? "") ? native.yamlRole : null
            };
        ...
        if (selected) {
            Object.assign(observation, {
                game: selected.game,
                gameDataRole: selected.gameDataRole,
                main: selectedFile(selected.main),
                gameFile: selectedFile(selected.gameFile),
                diagnostics: diagnostics(root, selected.diagnostics)
            });
        }
        if (snapshot) {
            observation.localIgnore = {
                state: token(snapshot.localIgnoreState),
                identity: identity(snapshot.localIgnoreIdentity, ignorePath)
            };
            observation.snapshot = {
                classicVersion: snapshot.yamlData.classicVersion, gameRootName: snapshot.yamlData.gameRootName,
        ...
        if (recovery && fixture.recoveryAction === "proceed") {
            ...
            observation.localIgnore = {
                state: token(proceeded.localIgnoreState),
                identity: identity(proceeded.localIgnoreIdentity, ignorePath)
            };
            observation.snapshot = {
                classicVersion: proceeded.yamlData.classicVersion,
                gameRootName: proceeded.yamlData.gameRootName,
                ignoreList: proceeded.yamlData.ignoreList,
                simplifyRemoveList: proceeded.simplifyRemoveList
            };
        }
        ...
        if (recovery && fixture.recoveryAction === "reset") {
            ...
            if (outcome.status === classic.JsLocalIgnoreResetStatus.Conflict && outcome.conflict) {
                ...
                observation.recovery.decision = {
                    status: "conflict",
                    expectedIdentity: identity(conflict.expectedIdentity, ignorePath),
                    actualIdentity: conflict.actualIdentity == null ? null : identity(conflict.actualIdentity, ignorePath),
                    backupPath: conflict.backupPath == null ? null : pathCarrier(root, conflict.backupPath)
                };
                ...
                observation.recovery.decision = {
                    status: "reset",
                    localIgnorePath: pathCarrier(root, reset.localIgnorePath),
                    malformedIdentity: identity(reset.malformedLocalIgnoreIdentity, ignorePath),
                    backupIdentity: identity(reset.backupIdentity, backupName),
                    replacementIdentity: identity(reset.replacementIdentity, ignorePath)
                };
                observation.diagnostics = diagnostics(root, reset.diagnostics);
                const resolved = reset.snapshot;
                observation.localIgnore = {
                    state: token(resolved.localIgnoreState),
                    identity: identity(resolved.localIgnoreIdentity, ignorePath)
                };
                observation.snapshot = {
                    classicVersion: resolved.yamlData.classicVersion,
                    gameRootName: resolved.yamlData.gameRootName,
                    ignoreList: resolved.yamlData.ignoreList,
                    simplifyRemoveList: resolved.simplifyRemoveList
                };
