...
export async function observeXseOperations(fixture: JsonObject): Promise<JsonObject> {
    ...
    const kinds: Record<string, classic.JsXseType> = {
        F4SE: classic.JsXseType.F4se,
        F4SEVR: classic.JsXseType.F4sevr,
        SKSE: classic.JsXseType.Skse,
        SKSE64: classic.JsXseType.Skse64,
        SKSEVR: classic.JsXseType.Sksevr,
        SFSE: classic.JsXseType.Sfse
    };
    ...
    try {
        ...
        const games: Record<string, classic.JsGameId> = {
            F4SE: classic.JsGameId.Fallout4,
            F4SEVR: classic.JsGameId.Fallout4Vr,
            SKSE64: classic.JsGameId.Skyrim,
            SFSE: classic.JsGameId.Starfield
        };
        ...
        return {
            typeName: classic.xseTypeName(kind),
            loaderName: classic.xseLoaderName(kind),
            dllPrefix: classic.xseDllPrefix(kind),
            installed: classic.isXseInstalled(root, kind),
            version: classic.detectXseVersion(join(root, classic.xseLoaderName(kind)), kind),
            info: {typeName: info.xseType, installed: info.installed, version: info.version ?? null},
            files: await Promise.all((await readdir(root)).sort().map(async name => ({
                path: name,
                hex: (await readFile(join(root, name))).toString("hex")
            })))
        };
    } finally {
