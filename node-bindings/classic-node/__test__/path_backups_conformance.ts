...
export async function observePathBackups(fixture: Record<string, any>): Promise<Record<string, any>> {
    ...
    try {
        ...
        await writeFile(log, fixture.log);
        await writeFile(source, Buffer.from(fixture.firstHex, "hex"));
        ...
        return {
            ...
            root: local(manager.backupRoot),
            directory: local(manager.getVersionPath(explicit)),
            created: local(created),
            firstHex: first,
            replacementHex: (await readFile(created)).toString("hex"),
            files
        };
    } finally {
