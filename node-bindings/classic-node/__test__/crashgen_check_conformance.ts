...
export async function observeCrashgenCheck(fixture: Record<string, any>): Promise<Record<string, any>> {
    ...
    const issues = (values: any[]) => values.map(value => ({
        path: relative(root, value.filePath).replaceAll("\\", "/"),
    ...
    try {
        ...
        const result = new JsCrashgenChecker(root, "Buffout4").check();
        ...
        return {
            ...
            name: report.crashgenName,
            config: report.configPath ? relative(root, report.configPath).replaceAll("\\", "/") : null,
            plugins: report.installedPlugins.sort(),
            beforeFiles: before,
