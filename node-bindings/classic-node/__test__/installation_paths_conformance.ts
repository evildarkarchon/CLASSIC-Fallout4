...
export async function observeInstallationPaths(fixture: JsonObject): Promise<JsonObject> {
    ...
    try {
        ...
        gameFinder.validateGamePath(game);
        docsFinder.validateDocsPath(docs);
        ...
        try {
            docsFinder.validateIniFiles(docs, ["Fallout4.ini"]);
        } catch (error) {
            if (!(error instanceof Error) || !error.message.includes("Fallout4.ini")) throw error;
        ...
        classic.checkDriveExists(root);
        classic.checkReadPermissions(game);
        classic.checkWritePermissions(game);
        ...
        try {
            classic.validateRequiredFiles(game, ["missing.ini"]);
        } catch (error) {
        ...
        return {
            gamePath: gameFinder.findGamePath(game, null), docsPath: docsFinder.findDocsPath(docs),
