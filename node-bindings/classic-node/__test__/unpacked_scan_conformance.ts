...
async function directories(root: string, prefix = ""): Promise<string[]> {
    ...
    for (const entry of await readdir(join(root, prefix), {withFileTypes: true})) {
        ...
        if (entry.isDirectory()) {
            result.push(path, ...await directories(root, path));
        }
    }
    return result.sort();
}
...
function issues(value: Record<string, any>, root: string): Record<string, any> {
    const fields = {
        ...
        formats: value.texFrmt,
        sounds: value.sndFrmt,
        scripts: value.xseFile,
        previs: value.previs
    };
    return {
        ...Object.fromEntries(Object.entries(fields).map(([name, values]) => [name, (values as string[]).map(value => value.replaceAll("\\", "/")).sort()])),
        dds: value.ddsFiles.map((path: string) => relative(root, path).replaceAll("\\", "/")).sort()
    };
}
...
export async function observeUnpackedScan(fixture: Record<string, any>): Promise<Record<string, any>> {
    ...
    const target = (path: string) => {
        if (path.includes(":") || path.includes("\\") || path.split("/").some(part => ["", ".", ".."].includes(part))) throw new Error("invalid unpacked path");
    ...
    try {
        ...
        const result: Record<string, any> = {
            beforeFiles: await inventory(root),
            beforeDirectories: await directories(root)
        };
        const scanner = new JsUnpackedScanner();
        result.issues = issues(scanner.scanDirectory(root, fixture.scripts), root);
        ...
        result.directories = await directories(root);
        return result;
    } finally {
