...
export async function observeFormidFinding(fixture: Record<string, any>): Promise<Record<string, any>> {
    ...
    const lookup = mode === "disabled" ? JsFormIdValueLookup.disabled() : mode === "in-memory" ? JsFormIdValueLookup.inMemory(fixture.entries.map((entry: Record<string, any>) => ({
        formid: entry.formid,
        plugin: entry.plugin,
        value: entry.value ?? undefined,
        operationalFailure: entry.failure ?? undefined
    }))) : null;
    ...
    try {
        ...
        return {
            ...
            findings: result.findings.map(finding => ({
                ...
                plugin: finding.plugin ?? null,
                status: finding.valueLookupStatus,
