...
export async function observeUpdateServices(fixture: JsonObject, scenarioId: string): Promise<JsonObject> {
    ...
    const config = JSON.stringify({
        github_api_base_url: `${base}/api`,
        notification_pages_url: `${base}/pages`,
    ...
    try {
                ...
                results.push({
                    status: {
                        ...
                        display: status.display ? {
                            ...
                            body: status.display.body,
                            ctaUrl: status.display.ctaUrl ?? null
                        } : null,
        ...
        const files = readdirSync(cache).sort().map(name => ({
            path: `cache/${name}`,
            hex: readFileSync(join(cache, name)).toString("hex")
        }));
