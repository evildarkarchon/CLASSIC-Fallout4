...
export async function observeFileFingerprint(fixture: Record<string, any>): Promise<Record<string, any>> {
  ...
  try {
    ...
    api.clearHashCache();
    const files = [];
    for (const name of (await readdir(root)).sort()) files.push({ path: name, bytes: [...await readFile(join(root, name))] });
    return { hash, error, encoding: fixture.bytes === null ? null : api.detectEncoding(target), batch, map, cache, reset, cleared: stats(), files };
  } finally {
