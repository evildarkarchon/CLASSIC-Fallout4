...
import {checkEnb, JsEnbChecker, JsIniValidator, JsLogProcessor, processGameLogs} from "../index.js";
...
export async function observeScanGame(fixture: JsonObject): Promise<JsonObject> {
    ...
    try {
        ...
        if (operation === "process-logs") {
            ...
            return {
                report: report.replaceAll(root, "<ROOT>").replaceAll("\\", "/"),
                beforeFiles: before.files,
                beforeDirectories: before.directories,
                files: after.files,
        ...
        if (operation === "validate-ini") {
            ...
            const issues = validator.detectAllIssues(validator.scanConfigFiles(root)).map(issue => ({
                ...issue,
                filePath: relative(root, issue.filePath).replaceAll("\\", "/")
            }));
