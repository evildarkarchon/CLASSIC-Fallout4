...
function writeWorkspaceDataRoot(workspace: string): string {
  ...
  writeFileSync(join(workspace, "CLASSIC Ignore.yaml"), CLI_IGNORE_YAML, "utf8");
  writeFileSync(
...
function runCli(args: string[], workingDirectory: string, env?: NodeJS.ProcessEnv): CliResult {
...
describe("classic-node CLI", () => {
  ...
  test("returns success when no logs are found", () => {
    ...
    const result = runCli(["--scan-path", scanDir, "--game-version", "auto"], workspace);
