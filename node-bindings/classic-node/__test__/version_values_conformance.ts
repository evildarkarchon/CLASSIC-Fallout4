...
export function observeVersionValues(family: string, fixture: Record<string, any>): Record<string, any> {
  ...
  if (family === "game-version-parse") {
    try { return { parsed: classic.parseGameVersion(request.version) }; }
    catch (error) {
  ...
  try {
    ...
    return { variants, files: readdirSync(root).sort().map(path => ({ path, content: readFileSync(join(root, path), "utf8") })) };
  } finally {
