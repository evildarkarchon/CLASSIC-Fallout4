...
export async function observeDdsHeader(fixture:Record<string,any>):Promise<Record<string,any>>{
  ...
  const root=await mkdtemp(join(tmpdir(),"classic-dds-node-"));const path=join(root,"texture.dds");
  try{
    await writeFile(path,Buffer.from(fixture.bytes));const analyzer=new JsDdsAnalyzer("Fallout4");
    return {issues:analyzer.validateFile(path).map(issue=>issue.message),
