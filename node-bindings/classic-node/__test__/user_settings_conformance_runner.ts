    ...
    for (const [path, value] of Object.entries(fields)) {
        ...
        switch (path) {
            case "/CLASSIC_Settings/Update Check":
                update.updateCheck = value as never;
                break;
            case "/CLASSIC_Settings/Update Source":
                update.updateSource = value as never;
                break;
            case "/UI/preferences/auto_switch_after_scan":
                update.autoSwitchAfterScan = value as never;
                break;
            case "/CLASSIC_Settings/Managed Game":
                update.managedGame = value as never;
                break;
            case "/CLASSIC_Settings/Game Version":
                update.gameVersionSelection = value as never;
                break;
            case "/CLASSIC_Settings/Game Folder Path":
                update.gameRoot = value as never;
                break;
            case "/CLASSIC_Settings/Game EXE Path":
                update.gameExecutable = value as never;
                break;
            case "/CLASSIC_Settings/Documents Folder Path":
                update.documentsRoot = value as never;
                break;
            case "/CLASSIC_Settings/INI Folder Path":
                update.iniFolder = value as never;
                break;
            case "/CLASSIC_Settings/MODS Folder Path":
                update.modsFolder = value as never;
                break;
            case "/CLASSIC_Settings/FCX Mode":
                update.fcxMode = value as never;
                break;
            case "/CLASSIC_Settings/Simplify Logs":
                update.simplifyLogs = value as never;
                break;
            case "/CLASSIC_Settings/Show Statistics":
                update.showStatistics = value as never;
                break;
            case "/CLASSIC_Settings/Show FormID Values":
                update.formidValueLookup = value as never;
                break;
            case "/CLASSIC_Settings/FormID Databases":
                update.formidDatabases = value as never;
                break;
            case "/CLASSIC_Settings/Move Unsolved Logs":
                update.moveUnsolvedLogs = value as never;
                break;
            case "/CLASSIC_Settings/Unsolved Logs Destination":
                update.unsolvedLogsDestination = value as never;
                break;
            case "/CLASSIC_Settings/SCAN Custom Path":
                update.customScanInput = value as never;
                break;
            case "/CLASSIC_Settings/Papyrus Log Path":
                update.papyrusLogPath = value as never;
                break;
...
async function installFixture(plan: RunPlan, scenario: Scenario, root: string, item: {
    fixtureRef: string;
    path: string
}): Promise<void> {
    if (!scenario.fixtureRefs.includes(item.fixtureRef)) throw new Error("fixtureRef is not declared by the scenario");
...
async function executeGeometry(plan: RunPlan, scenario: Scenario): Promise<JsonObject> {
    ...
    try {
        ...
        try {
            const outcome = classic.commitFrontendGeometryTransition(root, snapshot.revision, classic.JsGuiWindow.Main, {
                maximized: false,
                width: 900,
                height: 650
            });
            ...
            transition = {
                ...
                code: null,
                hasMessage: null,
                revisionMatches: outcome.revision === "sha256:" + createHash("sha256").update(content).digest("hex")
            };
        } catch (error: any) {
            ...
            transition = {
                status: "error",
                code: error.code,
                hasMessage: typeof error.message === "string" && error.message.trim().length > 0,
                revisionMatches: null
            };
        }
        const current = classic.openUserSettings(root).frontendState.windowGeometry.mainTab;
        const tree = await snapshotTree(root);
        const files = [...tree.directories.map(path => ({
            path,
            kind: "directory"
        })), ...Array.from(tree.files.keys()).map(path => ({
            path,
            kind: "file"
        }))].sort((a, b) => a.path.localeCompare(b.path));
        return {
            transition,
            geometry: {maximized: current.maximized, width: current.width, height: current.height},
            files
        };
    } finally {
        await rm(root, {recursive: true, force: true});
...
async function executeLegacyImport(plan: RunPlan, scenario: Scenario): Promise<JsonObject> {
    ...
    try {
        for (const item of scenario.input.installationData) await installFixture(plan, scenario, root, item);
        const classic = await import("../index.js");
        const original = await readFile(join(root, "CLASSIC Settings.yaml")),
            legacy = await readFile(join(root, "state.json"));
        ...
        const published = await readFile(join(root, "CLASSIC Settings.yaml"));
        const backup = await readFile(receipt.backupPath);
        const sourceRevision = "sha256:" + createHash("sha256").update(legacy).digest("hex"),
            baseRevision = "sha256:" + createHash("sha256").update(original).digest("hex"),
            publishedRevision = "sha256:" + createHash("sha256").update(published).digest("hex");
        ...
        const observed = {
            status: outcome.status,
            sourcePath: relativePath(root, receipt.sourcePath),
            backupPath: relativePath(root, receipt.backupPath),
            settingsPath: relativePath(root, receipt.settingsPath),
            settingsBackupPath: receipt.settingsBackupPath == null ? null : relativePath(root, receipt.settingsBackupPath),
            sourceRevisionMatches: receipt.sourceRevision === sourceRevision,
            backupRevisionMatches: receipt.backupRevision === "sha256:" + createHash("sha256").update(backup).digest("hex") && backup.equals(legacy),
            baseRevisionMatches: receipt.baseSettingsRevision === baseRevision,
            publishedRevisionMatches: receipt.publishedSettingsRevision === publishedRevision,
            inapplicable: {
                classification: outcome.classification ?? null,
                revision: outcome.revision ?? null,
                expectedRevision: outcome.expectedRevision ?? null,
                actualRevision: outcome.actualRevision ?? null
            },
            tui: {
                activeTab: tui.activeTab,
                resultsPanelWidth: tui.resultsPanelWidth,
                sortAscending: tui.sortAscending,
                origins: [tui.activeTabOrigin, tui.resultsPanelWidthOrigin, tui.sortAscendingOrigin]
            }
        };
        ...
        return {
            ...
            restore: {
                ...
                revisionMatches: restored.revision === baseRevision && (await readFile(join(root, "CLASSIC Settings.yaml"))).equals(original),
                expectedRevision: restored.expectedRevision ?? null,
                actualRevision: restored.actualRevision ?? null
            },
            files: Array.from(tree.files).sort(([a], [b]) => a.localeCompare(b)).map(([path, bytes]) => ({
                path,
