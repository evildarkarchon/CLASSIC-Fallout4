...
test("vocabulary transport resolves PascalCase enum carriers and preserves mixed outcomes", () => {
  expect(observeVocabulary("config-vocabulary", {
    ...
    entries: [
      { token: "updated", label: classic.installedYamlDataProvenanceLabel(classic.JsInstalledYamlDataProvenance.Updated), rejected: false },
      { token: "not_a_real_token", label: null, rejected: true },
      { token: "bundled", label: classic.installedYamlDataProvenanceLabel(classic.JsInstalledYamlDataProvenance.Bundled), rejected: false },
    ],
  });
  expect(observeVocabulary("scan-run-vocabulary", {
    ...
    entries: [{ token: "proceed_without_ignore", label: classic.scanRunLocalIgnoreYamlDataStateLabel(classic.JsScanRunLocalIgnoreState.ProceedWithoutIgnore), rejected: false }],
  });
});
...
test("vocabulary runner preserves unknown-token rejection without fixtures", async () => {
  ...
  try {
    ...
    await writeFile(planPath, JSON.stringify({
      ...
      scenarios: [{ id: "unknown", capabilityIds: [], fixtureRefs: [], action: "vocabulary.resolve",
        input: { operation: "installed_yaml_data_provenance_label", tokens: ["not_a_real_token", "not_a_real_token"] } }],
    }));
