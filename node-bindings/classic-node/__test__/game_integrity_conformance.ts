...
export async function observeIntegrity(fixture: any): Promise<Record<string, unknown>> {
    ...
    try {
        ...
        const checker = new classic.JsGameIntegrityChecker({
            gameExePath: join(root, fixture.exe),
            validExeHashes: fixture.hashes,
            rootName: fixture.rootName,
            steamIniPath: fixture.steamIni == null ? undefined : join(root, fixture.steamIni),
            rootWarn: fixture.rootWarn ?? undefined
        });
        ...
        async function visit(directory: string): Promise<void> {
            for (const entry of await readdir(directory, {withFileTypes: true})) {
                ...
                if (entry.isSymbolicLink()) throw new Error("unexpected integrity symlink");
                if (entry.isDirectory()) await visit(path); else files.push({
