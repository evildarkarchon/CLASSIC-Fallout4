...
import ClassicLib # Added for GlobalRegistry
...
def create_mock_yaml_config():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (base_path / "CLASSIC Ignore.yaml").write_text(
            "CLASSIC_Ignore_Fallout4:\n"
            "  - \"ignore_pattern_1\"\n"
            "  - \"ignore_pattern_2\"\n"
        )
...
class TestYamlData:
    ...
    def test_init_and_property_access(self, create_mock_yaml_config: Path, setup_global_registry):
        ...
        """
        yaml_dirs = [str(create_mock_yaml_config), str(create_mock_yaml_config / "CLASSIC Data")]
    ...
    def test_init_missing_yaml_dir(self, create_mock_yaml_config: Path, setup_global_registry):
        ...
        with pytest.raises(classic_config.RustConfigIOError, match="YAML file not found"):
            # Test with missing data directory
            yaml_dirs = [str(create_mock_yaml_config), str(create_mock_yaml_config / "YAML" / "NonExistent")]
            game = GlobalRegistry.get_game()
            vr_mode = bool(GlobalRegistry.get_vr())
    ...
    def test_init_malformed_yaml(self, create_mock_yaml_config: Path, setup_global_registry):
        ...
        with pytest.raises(classic_config.RustConfigParseError, match="Parse error"):
    ...
    def test_init_invalid_game(self, create_mock_yaml_config: Path, setup_global_registry):
        ...
        with pytest.raises(classic_config.RustConfigIOError, match=r"YAML file not found: .*CLASSIC InvalidGame.yaml"):
            classic_config.YamlData(yaml_dirs, game, vr_mode)
