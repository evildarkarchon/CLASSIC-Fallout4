...
# =============================================================================
# AsyncBridge Thread Safety Tests (from test_thread_safety_stress.py)
# =============================================================================


@pytest.mark.stress
@pytest.mark.slow
class TestAsyncBridgeThreadSafety:
    """Test AsyncBridge thread safety under concurrent access.

    These tests validate that AsyncBridge properly maintains thread-local
    singleton instances and handles concurrent async operations correctly.
    """

    def test_async_bridge_thread_local_singleton(self):
        """Test that AsyncBridge is thread-safe singleton per thread.

        Note: AsyncBridge maintains one instance PER THREAD (thread-local singleton pattern).
    ...
    def test_concurrent_data_modification_synchronization(self):
        """Test thread safety when modifying shared data with proper synchronization."""
        counter = {"value": 0}
        lock = threading.Lock()

        def increment_counter():
            """Increment shared counter with proper synchronization."""
            for _ in range(1000):
                with lock:
                    counter["value"] += 1
        ...
        try:
            ...
            for _ in range(50):
                try:
                    ...
                    pass  # May hit concurrency limit
