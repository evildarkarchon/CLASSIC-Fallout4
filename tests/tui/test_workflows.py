    ...
    async def test_complete_crash_scan_workflow(self):
        ...
        async with app.run_test() as pilot:
            ...
            with patch("ClassicLib.TUI.handlers.scan_handler.ClassicScanLogs") as mock_scanner:
                ...
                mock_instance.scan_logs = Mock(
                    return_value=("Analysis Complete", ["Found 3 crash logs", "2 critical issues", "1 warning"])
                )
    ...
    async def test_complete_papyrus_monitoring_workflow(self):
        ...
        async with app.run_test() as pilot:
            ...
            with patch("ClassicLib.TUI.handlers.papyrus_handler.papyrus_logging") as mock_logging:
                ...
                # Step 2: Verify monitoring started automatically
                assert papyrus_screen.is_monitoring is True

                # Step 3: Toggle Unicode mode
                await pilot.press("u")
                await pilot.pause()
