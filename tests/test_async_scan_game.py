...
class TestAsyncScanModsArchived:
    ...
    async def test_scan_mods_archived_async_with_valid_ba2(
        self, mock_settings, mock_paths, mock_scan_settings, 
...
class TestAsyncCheckLogErrors:
    ...
    @pytest.mark.asyncio
    async def test_check_log_errors_async_with_errors(
        self, mock_settings, mock_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_log_errors_async_concurrency(
        self, mock_settings, mock_paths
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_log_errors_async_with_unreadable_file(
        self, mock_settings, mock_paths
    ):
...
class TestAsyncScanModsUnpacked:
    ...
    async def test_scan_mods_unpacked_async_basic(
        ...
        (mod1_dir / "test.dds").write_bytes(b"DDS \x00\x00\x00\x00\x00\x00\x00\x00\x00\x04\x00\x00\x00\x04\x00\x00")  # Valid DDS header 1024x1024
