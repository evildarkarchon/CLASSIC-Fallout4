...
impl SettingsValidator {
    ...
    pub fn scan_buffout_looksmenu_setting(
        ...
        if let Some(f4ee) = crashgen.get_item("F4EE")? {
            let f4ee_enabled = f4ee.extract::<bool>().unwrap_or(false);
    ...
    pub fn check_disabled_settings(
        ...
        for (key, value) in crashgen.iter() {
            let setting_name = key.extract::<String>()?;
...
mod tests {
    ...
    fn test_achievements_validation() {
        Python::attach(|py| {
            ...
            let crashgen = PyDict::new(py);
            crashgen.set_item("Achievements", true).unwrap();
    ...
    fn test_memory_management_xcell_conflict() {
        Python::attach(|py| {
            ...
            let crashgen = PyDict::new(py);
            crashgen.set_item("MemoryManager", true).unwrap();
    ...
    fn test_archive_limit_warning() {
        Python::attach(|py| {
            ...
            let crashgen = PyDict::new(py);
            crashgen.set_item("ArchiveLimit", true).unwrap();
