...
json execute_version_values_scenario(const json& plan, const json& scenario) {
    ...
    if (family == "game-version-parse") {
        ...
        if (classic::version_registry::is_null_version(v.major, v.minor, v.patch) !=
            (v.major == 0 && v.minor == 0 && v.patch == 0))
            throw RunnerError("null sentinel query disagrees with parsed version components");
    ...
    // First registry/config access must bind the singleton to the authored YAML.
    ...
    for (const auto v : {Version::Original, Version::NextGen, Version::AnniversaryEdition, Version::Vr}) {
        if (family == "fallout4-paths") {
            const auto metadata = classic::version_registry::version_registry_get_by_id(classic::version_registry::fallout4_version_registry_id(v));
            const auto resolved = classic::path::resolve_fallout4_exe_name(classic::version_registry::fallout4_version_as_str(v));
            if (std::string(resolved) != std::string(metadata.docs_name) + ".exe")
                throw RunnerError("config executable resolver disagrees with registry metadata");
        }
