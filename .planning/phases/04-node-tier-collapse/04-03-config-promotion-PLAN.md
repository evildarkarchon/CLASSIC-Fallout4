...
must_haves:
  truths:
    ...
    - "bun run parity:gate:local exits 0 at plan close; runtime_coverage_summary.json::per_owner.config.deferred drops to 0 (or ≤2 if any residuals surface that Plan 5/7 absorbs)."
    - "Every new row carries a `rustCrate` field per A3. **Cross-crate routing**: some proxy rows route to `classic-config-core` (e.g., `ConfigError`), others route to `classic-crashgen-settings-core` (e.g., `SuspectErrorRule`, `ModSolutionCriteria`). The routing is NOT uniform — the `rustCrate` value comes from the live `rust_api_surface.json` lookup per symbol, not a hard-coded set. This bullet REPLACES the earlier 'all new rows have rustCrate: classic-config-core' wording, which contradicted Task 1 Step 2/3 cross-crate routing."
  artifacts:
