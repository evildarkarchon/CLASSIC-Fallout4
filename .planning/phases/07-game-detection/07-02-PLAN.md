...
must_haves:
  ...
  artifacts:
    - path: "ClassicLib/support/xse.py"
      provides: "Thin wrapper delegating to Rust XseChecker"
...
Output: Rust EnbChecker added to classic-scangame; XSE/ENB validation wired to Rust; GlobalRegistry keys for validation flags; xse.py simplified to thin Rust wrapper.
</objective>
...
```
  ...
  <done>GlobalRegistry.Keys contains XSE_VALID, ENB_PRESENT, XSE_VERSION, GAME_VERSION_DETECTED; convenience methods is_xse_valid() and is_enb_present() exist</done>
</task>
...
<task type="auto">
  <name>Task 4: Wire xse.py to use Rust XseChecker and add validation integration</name>
...
3. **Update `xse_check_integrity()` to use Rust:**
```python
...
5. **Keep xse_check_hashes() if it exists** - it may have different purpose (checking DLL hashes).
  </action>
  <verify>
```bash
# Verify Rust imports
...
# Test imports work
uv run python -c "from ClassicLib.support.xse import xse_check_integrity, enb_check_presence; print('XSE/ENB functions OK')"

# Run existing tests
uv run pytest tests/ -k "xse" -v --tb=short
```
  </verify>
  <done>xse.py uses Rust XseChecker and EnbChecker; validation gated by FCX Mode; results stored in GlobalRegistry.Keys.XSE_VALID and ENB_PRESENT</done>
</task>
...
<success_criteria>
- GlobalRegistry.Keys contains XSE_VALID, ENB_PRESENT, XSE_VERSION, GAME_VERSION_DETECTED
