...
*Added 2026-04-09 after gsd-phase-researcher run (artifact: `.planning/phases/04-node-tier-collapse/04-RESEARCH.md`, commit `18dafd28`). These amendments lock the 5 open questions surfaced during research and incorporate two material CONTEXT corrections the researcher traced to live source. Amendments A1–A7 take precedence over any conflicting D-XX decision above; the original D-XX text is preserved unedited for audit history.*

- **A1 — `RUST_TARGET_CRATES` expands to 19, not 18 (supersedes D-TOOL-01).** `classic-crashgen-settings-core` IS included. The researcher traced `ClassicLib-rs/node-bindings/classic-node/src/crashgen_rules.rs` as a direct NAPI binding for that crate — 8 `#[napi(object)]` types + 2 helper functions covering 7 aux deferred entries. D-TOOL-01's "excluding `classic-crashgen-settings-core`" text is wrong; the exclusion was a Phase 3 inheritance mistake. Plan 1 adds `classic-crashgen-settings-core` to `RUST_TARGET_CRATES` alongside the other 9 new entries. Plan 5 (aux promotion) incorporates the 7 resulting crashgen_rules rows. Rationale: matches live source reality; keeps the 19-crate set symmetric with Phase 3; prevents silent undercounting in the A10 sizing report.

- **A2 — `GLOBAL_FCX_HANDLER` is NOT promoted (scanlog net rows: 67 → 66).** Replicates Phase 3 R9 precedent: static handler singletons are not clean parity targets because their instance lifecycle does not map to a callable contract row. Plan 2's task budget uses **66 rows**, not 67. The backlog row for `GLOBAL_FCX_HANDLER` is deleted in Plan 6's final cleanup cascade (same commit that empties `deferred_runtime_backlog.json::entries`).

- **A3 — `rustCrate` field policy: new rows only.** Plans 2–5 and HARM-01/02 (Plan 4) add a `rustCrate` field to every NEW contract row they write. The 261 existing `tier1Mappings` rows are left untouched. Rationale: Plan 1's `validate_contract_surface()` diagnostic message becomes more actionable for newly-added rows (the diagnostic can say "add `pub use` to `<crate>/lib.rs`") without forcing a churn commit across 261 unrelated rows. Plan 1's guard must handle both `rustCrate`-present and `rustCrate`-absent rows gracefully — absent rows fall back to the generic "symbol missing" message.

- **A4 — NODE-06 success criterion: `deferred_total == 0` is the sole gate.** `tier2_gap_total` self-resolves in Plan 6's atomic cleanup commit (deleting the `gap_type=rust_unmapped`/`node_unmapped` branches in `generate_baseline.py` makes the field structurally unreachable). No separate verification step for `tier2_gap_total` is required. Plan 6's acceptance criteria reference only `deferred_total == 0` from `runtime_coverage_summary.json`.

- **A5 — Node owner labels match Phase 3's 19 distinct owners — do NOT collapse `perf`/`registry`/`shared`.** Plan 1 adds distinct `RUST_OWNER_BY_CRATE` entries for each newly-tracked crate and extends `SQUAD_BY_OWNER` to cover them (matching Phase 3's squad structure). Rationale: keeps the A10 sizing report's per-owner rows granular enough to spot uneven gap distribution across plans; preserves cross-phase consistency when comparing Phase 3 and Phase 4 tier1_gate_report output.

- **A6 — Plan 4 prerequisite: add `pub use` for `is_valid_executable_path` (CRITICAL).** The researcher found that `ClassicLib-rs/business-logic/classic-version-core/src/lib.rs` (line 43) re-exports `{PeVersionError, PeVersionResult, extract_pe_version}` but NOT `is_valid_executable_path`. Plan 4's first task MUST add `pub use pe_version::is_valid_executable_path;` to that re-export line. Without this, Plan 1's bidirectional `validate_contract_surface()` guard fails when the D-PE-03 contract row lands (the `rustSymbol` would not appear in `rust_api_surface.json` because the parser walks `lib.rs` re-exports, not sub-module public items). This is a load-bearing prerequisite, not a cleanup — Plan 4 sequences the re-export commit before the NAPI wrapper commit.

- **A7 — Plan 2 scanlog promotion: 58 of 66 rows need `@rust`-suffix proxy pattern (not direct `#[napi]` wrappers).** The researcher verified that 58 of the 67 scanlog deferred rows (67 − 1 FCX exclusion = 66 net; of those 66, 58 Rust-only) have NO `bindingIdentifiers` and no matching `index.d.ts` export — they are Rust-only public symbols that are not, and will not be, wrapped at the NAPI layer. D-SCANLOG-01 (implicit in §Plan 2 scope) said "the wrapper types already exist (this phase exposes them via `#[napi]`)" — that assumption is wrong for 58/66 rows. Plan 2 MUST use the Phase 3 Plan 09b "Scenario E" `@rust`-suffix proxy-row pattern: a contract row with `rustSymbol: "classic_scanlog_core::parser::parse_line@rust"` and no `nodeExport` field, which the bidirectional guard treats as "Rust-side only, no Node surface expected". The 8 rows that DO have a Node wrapper target get normal `nodeExport` fields. Plan 2's task decomposition splits these two groups explicitly. Reference: `.planning/phases/03-python-tier-collapse/03-02-scanlog-wave1-parsing-primitives-SUMMARY.md` §"Scenario E proxy rows".
